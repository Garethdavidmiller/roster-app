...
- **`.pc-side { grid-row: 4 / 6 }`** — span ONLY the two rows the left cards (Hours row 4, Settings row 5) occupy. A larger span (e.g. `4 / 8`) makes CSS grid distribute the sidebar's height across the empty rows 6–7, opening a **phantom full-width gap** above the footer (seen and fixed during the screenshot pass). `.pc-side > * { margin-bottom: 0 }` because the `main > *` reset only reaches direct children.
...
**Sticky take-home bar (`#stickyTotal`)** — fixed bottom bar on **all widths** (v16.12; desktop capped to `--content-max-width`, centred, rounded top corners). `IntersectionObserver` on `#netDisplay` shows it when the £ figure scrolls off; `body.sticky-active` adds bottom padding. On desktop the col-3 sticky rail keeps the full live breakdown visible for the ENTIRE work area (rows 4–7); the bar is the backstop for the footer/disclaimer rows below the rail's span. The two are temporally exclusive (rail's `#netDisplay` visible → bar hidden; rail scrolled past at the very bottom → bar shown).

**Desktop card placement (v16.67)** — explicit grid-row placement (mobile DOM order is untouched — every rule lives inside `@media (min-width:1024px)`): row 4 `#hoursCard` spans cols 1–2 · row 5 `#settingsCard` spans cols 1–2 · `.pc-side` col 3 `grid-row 4 / 6` (flex column: result + the four occasional cards) · `.guide-footer-link` row 6 · `.disclaimer` row 7. `align-self:start` on the work cards and the sidebar so an expanding accordion never stretches its neighbour. The occasional cards are NO LONGER 2-up in the work columns — they live stacked in the col-3 sidebar. (The old 2-up placement re-created voids whenever a tall expanded card sat beside a short one; stacking them in the sidebar sidesteps that.)
