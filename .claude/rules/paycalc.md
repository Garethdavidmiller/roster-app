...
**Back-pay estimate is AUTOMATIC (v16.00, D1 — owner-approved).** At init the coordinator calls `restoreBpState()`: **true** (a same-year saved blob exists — even all-blank, which is a deliberate clear that must stick) → recompute the saved figures; **false** (no saved state: first visit, or a new award year after the rollover discard) → `_refreshBackPayCard()` computes the DEFAULT pending-award estimate with the card closed — staff see the estimated lump on the right payslip without knowing the card exists. Every result-card surface labels it via `_bpIsEstimate` (from `calcBackPay`'s `bpIsEstimate` = award year unconfirmed): "Back pay lump sum (pay award — estimate)", "inc. estimated back pay", "✓ Includes estimated back pay lump sum…". Once the award is confirmed the same flow auto-computes the REAL lump (settled prefill + real payment period) with the plain labels. Opt-out = clear the card's boxes (the saved blank state prevents re-fill until the next April rollover).
