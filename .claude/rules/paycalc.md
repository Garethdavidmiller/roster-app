...
**Desktop workspace layout (v16.14)** — at ≥1024px paycalc is a **three-column** workspace: two wide work columns + a dedicated **live-estimate RAIL** (col 3). The two tall always-open cards (**Hours**, **Settings**) SPAN both work columns (`grid-column: 1 / 3`) so they stay wide (~650px) and wrap far less than the old ~440 right column — that width is the real scroll-reduction lever. The four occasional cards pair **2-up** below (Improve-Accuracy|Back-Pay, then HPP|Decimal) — all short when collapsed, so pairing them adds no navy void. The **result card is `position: sticky; top:20px` in col 3, its grid area spanning every work row (4–7)** so it is ALWAYS visible while editing — the full live breakdown, not just a number. **The rail is safe from the v16.12/v16.14-probe overlap because NOTHING else is ever in col 3**: a sticky card only overlaps when it shares a column with the cards it scrolls over, so a dedicated rail column removes that failure mode by construction. **Do not put another card in col 3, and do not make the result sticky while it shares a column** — either reintroduces the overlap (verified twice by `elementFromPoint` probes: a sticky result beside content slid over Settings, then over Improve-Accuracy).
...
**Desktop card placement (v16.14; supersedes the v16.12 full-width bands)** — explicit grid-row placement drives the whole thing (DOM order is fixed for the mobile stack): row 4 `#hoursCard` spans cols 1–2 · row 5 `#settingsCard` spans cols 1–2 · row 6 `#payslipCard`(col1)|`#backPayCard`(col2) · row 7 `#hppCard`(col1)|`#decimalConverterCard`(col2) · `.result-card` col 3 `grid-row 4 / 8` (sticky). `align-self:start` on every work card so an expanding accordion never stretches its row-neighbour. Only the four SHORT collapsibles are 2-up (similar heights → no void); the tall Settings card is full-width (2-up'ing it beside a short card was the abandoned v16.14 attempt that re-created the void). Mobile/tablet source order untouched — every rule lives inside `@media (min-width:1024px)`.
