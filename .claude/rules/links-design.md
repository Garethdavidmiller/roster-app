...
- **Delete has no undo.** Any designer or admin can permanently delete any design; the only guard is the confirm dialog, and there is no archive or ownership check. The only recovery route is a Firestore PITR restore (RECOVERY_RUNBOOK.md → "A Link Design was lost or corrupted"), which is only a route if PITR is actually enabled on the project. This was written at v19.39 as an accepted risk **at two designers, with a third as the trigger to revisit** — and the third (M. Robson) arrived at v19.40, so the trigger has fired: soft-delete is now an open question, not a settled one. Deliberately not built in the same change as granting access, which would have been scope the request didn't ask for.
