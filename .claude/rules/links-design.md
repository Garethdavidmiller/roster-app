...
The result is that **Saturday IS the weekday, with one late turned into a fourth closer** — exactly two rows differ, and that swap is the events lean in a form somebody can carry. The table went from three day-blocks to two, Mon–Sat and Sunday. **The cost was measured and is close to nothing**: held to within 5% of the shipped demand fit as a hard bound, each day's worst-staffed hour came out unchanged. Pushed further the search will trade a much worse weekday curve for a few more rows saved, and that trade was refused — the reasoning, the measured before/after, and the one fatigue factor that moved are in the module header. Pinned by `links-default-targets.test.mjs` as **ceilings**, so a shorter table is still an improvement and only giving readability back fails.
