...
- **And so is `NA`, for the same reason** (v24.20). It means **not available** — owner, 20 Sep 2026, "usually only falls on a Sunday as it is uncontracted". On Sunday not-available and not-working are the same thing BECAUSE nobody is contracted to be there, so `RD` is right; Mon–Sat the person IS contracted, the two stop being the same, and a silent `RD` would record them as resting. **The prompt does not decide this** — it used to (`Return "RD"`, unconditionally, every day), which is the same mistake the blank rule above exists to undo: a model answering what a cell MEANS when the meaning depends on which column it is in. It reports `NA` and `buildSafeEntries` decides, beside the blank rule. `NS` and `N/A` are spellings of the same code, not separate meanings.
