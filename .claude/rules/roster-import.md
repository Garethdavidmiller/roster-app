...
- **`NA` is an ABSENCE; `NS` is the Sunday code** (v24.22, correcting v24.20). The owner's corrected fact, 24 Sep 2026: `NA` means **not available and always means absent** — normally written Monday to Saturday, and a clerical error when it appears on a Sunday. `NS` is a different code, **not available on a Sunday**. So `NA`/`N/A` becomes the app's Absent day (`SICK`) Mon–Sat and, on a Sunday, the rest day a Sunday already is (a Sunday cannot hold an absence — the client's Sunday layer would strip it anyway — and the coordinator warns that it saw one); `NS` is `RD` on Sunday and a question for the admin on any other day. v24.20 had read the earlier remark that `NA` "usually only falls on a Sunday" as the rule, and sent every Mon–Sat `NA` to review as a question — a wrong fact applied carefully. **The prompt still does not decide this**: it reports the code and `functions/cell-day-rules.js` decides, beside the blank rule, because two of the three answers depend on the column.
