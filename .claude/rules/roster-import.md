...
- **`NA` is an ABSENCE; `NS` is the Sunday code** (v24.22, correcting v24.20). The owner's corrected fact, 24 Sep 2026: `NA` means **not available and always means absent** — normally written Monday to Saturday, and a clerical error when it appears on a Sunday. `NS` is a different code, **not available on a Sunday**. So `NA`/`N/A` becomes the app's Absent day (`SICK`) Mon–Sat and, on a Sunday, the rest day a Sunday already is (a Sunday cannot hold an absence — the client's Sunday layer would strip it anyway — and the coordinator warns that it saw one); `NS` is `RD` on Sunday and — the owner's second answer the same day, "NS on a weekday should also mean absent" — `SICK` on any other day, so the two codes carry one rule and differ only in which day is the clerical error. v24.20 had read the earlier remark that `NA` "usually only falls on a Sunday" as the rule, and sent every Mon–Sat `NA` to review as a question — a wrong fact applied carefully. **The prompt still does not decide this**: it reports the code and `functions/cell-day-rules.js` decides, beside the blank rule, because every one of these answers depends on the column.
...
a reason — `NA`/`N/A`/`NS` (the prompt reports the code and `cell-day-rules.js` decides it by the day) and `GER` (Gerrards Cross, a
