...
- **Searchability is not discoverability.** The drawer's cross-guide search only helps a reader who
  ...
  the search box.
- **An example of a search is not an answer to it** (v23.52). The Staff Guide's search tip names two
  things to try — "boxing day", "network railcard" — and naming them put those phrases into the
  cross-guide index as ordinary body text. A body match scores the same wherever it appears and ties
  break on index order, with the Staff Guide indexed first, so the top hit for `boxing day` became the
  paragraph telling you to search for it, ahead of the Boxing Day pay rows. Mark any such element
  `data-gs-example` and `scripts/guide-index-lib.mjs` drops it before extracting; the rule is pinned by
  contract 6 in `guide-index-parity.test.mjs`. Note what could not see it: the index was perfectly TRUE
  of the page, so the staleness contract passed.
...
- **Typeface (v18.79):** the shared Inter `@font-face` (same self-hosted `fonts/inter-latin.woff2` the app uses — SW-precached, CSP-clean) + the `--font-sans` token. Every guide body uses `var(--font-sans)` (railcard/fip/rangers in their own CSS; staff-guide/paycalc-guide via `guide-doc.css`) so the guides render the SAME brand typeface as the app — before this they fell back to the system stack and read subtly off-brand.
- **Header:** full-bleed sticky navy `.page-header`, `align-items: center`, `top: 0`, with `←` `.btn-back` (left) · title `<h1>` + `.sub` · `⤓ PDF` `.btn-pdf` (right, `margin-left:auto`). Top padding `max(14px, env(safe-area-inset-top))` for the iOS notch. **3px gold bottom rule (v18.79)** — the navy-and-gold brand echo; prints too (the header doubles as the railcard/fip/rangers print banner).
- **Chip bar alignment (polish round 2):** from 800px the bar's inline padding is the 760px content column's own text edge, so the first chip starts where the text does. It restates the 760/16 pair the three page CSS files declare — change them together.
- **Footer back link (polish round 2):** `.guide-back-link` is ONE recipe for railcard, rangers and FIP — centred, 44px tall, "← Back to calendar" to `./`, the last element of `<main>`, hidden in print by the shell. Do not restyle it per page; `e2e/pages.spec.js` fails if the three diverge.
...
- **Print:** `.page-header { position: static; print-color-adjust: exact }` and `.btn-back, .btn-pdf { display: none }`. railcard/fip/rangers print the sticky header as their title banner; on staff-guide/paycalc-guide the header is `.no-print` so these rules are inert there (they print their own in-document `.guide-header` banner instead — `print-color-adjust: exact` on that banner stays inline).

**Still per-page in each page's own CSS file (keep aligned by eye):**
- **Background:** flat `#f4f5f8` edge-to-edge. No white "page" card.
- **Content width:** `max-width: 760px`, centred. For `staff-guide.html`/`paycalc-guide.html` the 760px wrapper **and** the two-column `.cols` print grid live in the shared **`guide-doc.css`** (not the page's own file); `railcard-guide.html`/`fip-guide.html`/`rangers-guide.html` set their own 760px single-column width in their page CSS.
- **Safe-area:** side insets `max(16px, env(safe-area-inset-*))` on the content wrapper; bottom `max(40px, env(safe-area-inset-bottom))`.
- **PDF button markup:** `<button id="savePdfBtn" class="btn-print btn-pdf">⤓ PDF</button>`. Wired by `guide-print.js` (`.btn-print`) on staff-guide/paycalc-guide/fip; `railcard-guide.js` and `rangers-guide.js` wire `#savePdfBtn` themselves.
...
- **£ / ⊘ tokens in the When row.** £ = the discount still applies, subject to a minimum fare. ⊘ = the discount is not available yet. Both symbols defined in the key strip (with ↳, follows the ticket's own Off-Peak rule) — keep both.
...
- **Three-layer breakdown for "varies by route" time rules (v17.48).** Where a card's Mon–Fri cutoff is really a *local shortcut* for a national "morning-peak, off-peak-varies-by-route" rule, the card replaces its `.rc-chiltern` prose note with a `.rc-layers` block of three labelled rows — `.rc-layer-lbl official` (navy: the national rule) / `local` (amber: Chiltern's directional Off-Peak rule, set out in the "Marylebone mornings" panel — never a clock time) / `check` (red: what the retail system confirms). The page intro teaches the model once, with `.tl-tag` chips (the separate `.time-explainer` box it once had is gone, and its CSS went at the v24.28 review). **Applied ONLY to the genuinely-varies cards — Family & Friends and Senior.** Do NOT blanket every card with it: cards whose time IS fixed (Network 10:00, Gold Card 04:30–09:29) keep the simple note, and adding three layers everywhere would wreck the glanceable one-card scan the guide depends on. This is the durable fix for the root cause the v17.45 audit flagged (a local shortcut masquerading as a universal rule).
