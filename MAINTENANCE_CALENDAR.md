...
| When the RMT award lands · **VAL-PAY-002 check: 28 Aug 2026** (warning point 27 Aug — the row above had no date until v21.56, and an undated check is a check nobody's calendar fires for) | — | **Update `GRADES` / `AWARD_RATES`** with the confirmed rates. Evidence class B required — do not enter a rumoured figure. The 28 Aug 2026 step is `VAL-PAY-002` — check it off the payslip rather than assuming it landed | `.claude/rules/paycalc.md` · `VAL-PAY-002` |
