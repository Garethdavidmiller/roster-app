    ...
    describe('asking to be taken off a rostered Sunday (v23.80)', () => {
        ...
        test('and it is type-checked, like every other field', () => {
            for (const bad of ['yes', 1, null, {}]) {
                const r = C.normaliseDays(week({ mode: 'all_day', releaseRequested: bad }), DATES);
        ...
        test('THE SERVER DOES NOT CHECK THE ROSTER, and that is deliberate', () => {
            ...
            // it means something; the server's job is to store what was said, faithfully.
            const r = C.normaliseDays(week({ mode: 'all_day', releaseRequested: true }), DATES);
            assert.equal(r.ok, true, 'every date in the week is accepted, Sunday or not');
        });
    });
