...
// Pages mirror publishes the repo root with no ignore list, so a committed file is served there
// whatever `firebase.json` says). So on any checkout but the owner's it is simply not present.
//
// THE DANGEROUS SHAPE HERE IS SILENCE, NOT FAILURE. Every payslip assertion below is generated by a
// LOOP over the fixture's rows, so a missing fixture does not fail anything — it registers no tests
// at all, and the run reports a smaller green number that nobody reads. That is this repo's own
// named blind spot (a rule can be perfect and never called), and it would be self-inflicted here.
//
// So absence registers ONE test that announces itself, names what is not being verified, and says
