...
- **AI model:** the `CLAUDE_MODEL` constant in `functions/index.js` — currently `claude-sonnet-5`, `max_tokens: 24000`, `thinking: { type: 'enabled', budget_tokens: 8000 }`, called via **streaming** (`client.messages.stream(...).finalMessage()`), `timeoutSeconds: 300`. Note the model is an unversioned alias (it tracks the latest Sonnet 5 snapshot), not a dated pin; switch to a dated snapshot if reproducibility matters. **Thinking was RE-ENABLED (v16.27) for reading accuracy** — reading a roster table is a spatial column-alignment task, and with thinking OFF Sonnet 5 intermittently dropped a blank Sunday cell and shifted the whole row one day left (the reported "day-shift"). The budget lets it align cells to day headers before committing. Thinking was previously disabled only because default *adaptive* thinking + Sonnet 5's ~30% higher token count truncated the JSON past the old `max_tokens=8192`; that is avoided by keeping the OUTPUT budget (`max_tokens − budget = 16000`) unchanged and streaming so a longer thinking pass can't hit the non-streaming request timeout. The response handler skips leading thinking blocks. **Rollback:** set `thinking: { type: 'disabled' }` + `max_tokens: 16000` (one line). A deterministic server-side safety net (`applySundayScanCorrections` — right-shifts a clean blank-Sunday left-shift) backs this up regardless of the model.
...
- **`"HA"` (hospital appointment) and `"OD"` (paid absence / long-term sick marking) → `"SICK"`** (v15.45). On a base REST day the review normalises them to RD (never written; a stale imported one REMOVE_IMPORTs on re-upload) — full-pay absence only applies to rostered days. Sundays: blocked like all absence
- `"AL"`, `"A/L"`, `"A.L."` all mean annual leave — return `"AL"`
- **Other family (v15.34; Team Day added v15.51, OTHER_PLAN.md):** `"TRG"`/`"TRAINING"`/`"TRAIN"` → `"TRG"`; `"INDUCTION"`/`"IND"` → `"IND"`; `"ASSESSMENT(S)"`/`"ASSESS"` → `"ASSESS"`; `"TEAM DAY"`/`"TEAM DAYS"`/`"TEAM"` → `"TEAM"` (the one multi-word roster label); an RDW marker either side (`"TRG RDW"`, `"RDW TRG"`) is preserved as the canonical `"FLAVOUR RDW"`. Saved as `type: 'other'` with the value verbatim. An Other cell WITH times is unexpected (rosters never set them) → UNREADABLE review row. **An Other day on a Sunday is invalid** — normalised to RD like AL/SICK
