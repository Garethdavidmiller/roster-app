...
| **VAL-AL-001** | **AL recorded over a voluntary RDW consumes zero entitlement** (v21.55, `isContractedWorkOverride`: `shift` charges, `rdw` does not). The conceptual half is inferred, not confirmed: declining voluntary overtime is not spending contractual leave — but the app will still happily DISPLAY "AL" on a day it simultaneously charges nothing for, and the cleaner workflow may be "remove the RDW, never book AL over it". Which of those H. Croft actually practises decides whether the current shape is right, and it is a railway-workflow question the code cannot answer. | **Asking H. Croft one question:** *if somebody agreed RDW and later cannot work it, is that ever recorded as annual leave — and if so, does it deduct a day?* The Aug 2026 external review posed it in exactly this form | Wrong in one direction: a day recorded as AL that payroll DOES deduct would leave the app's balance one day generous per occurrence — the same drift v21.55 existed to stop, reintroduced by policy rather than code | `al-entitlement.js` (`consumesEntitlement`) · `override-utils.js` (`isContractedWorkOverride`) |
