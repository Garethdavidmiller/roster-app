 ...
 * Behaviour is UNCHANGED: init() runs the exact code that used to run at module scope, exactly once
 * (ES modules are deferred, so the timing is identical). The in-place-login re-invocation still
...
export function init() {
    ...
    // ============================================
    initAdminWeekSwipe({
        weekGrid: /** @type {HTMLElement} */ (weekGrid),
        ...
        onWeekCommitted: (iso) => {
            lastFieldDate = iso;
            updateWeekNavLabel(iso);
            updateALBanner();
            updateALBookedBox();
            updateSickBookedBox();
            userMadeChanges = false;
        },
        onSettled: () => { resetBulkPills(); updateSaveBtn(); },
        shiftWeek,
    });
    ...
    // WEEK GRID
    // ============================================
    // updateWeekNavLabel, renderWeekGrid, activateRow,
