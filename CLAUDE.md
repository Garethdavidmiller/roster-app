...
| **AL on a rest day is ASKED about, never defaulted (v23.75)** | Leave on a base-roster rest day costs no entitlement — that is what stops a stray booking and a declined RDW charging somebody a day — **unless the member SWAPPED working days onto it**, which only the person booking knows. So both booking paths ASK, per affected day, and refuse to save unanswered; a declared swap is recorded as `replacedType: 'shift'`, which the rules already permit, so this needs no schema or rules change. **Ask only when a day is affected, and never default** — both halves are the owner's, and a silent default is what lost three of one member's days. **"Rest day — free" means NOTHING IS WRITTEN, on both surfaces** (v23.88): until then the grid wrote leave costing nothing where the card wrote no document at all, so one answer produced two outcomes. The save receipt NAMES the days it left alone — a save may not do nothing silently. **And the answer must reach every layer, not just the write** (v23.79): preview, over-entitlement warning and save all read ONE projection. Never wire any of it to the Absence card. Reasoning: `al-swapped-days.js` (which days are asked) and `admin-al-projection.js` (what the answers mean, and why the grid reads `answeredFree` rather than `skipped`). |
