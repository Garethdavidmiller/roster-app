...
├── links-compare.js        ← links.html compare mode — two designs side-by-side with a gold-outline diff (extracted from links-app.js v17.71). `initLinksCompare(deps) → { toggleCompareMode, selectCompareDesign, renderCompare, isCompareMode, getCompareId, resetCompare }`. **Single source of truth for compareMode/compareDesignId** — links-app.js no longer stores them, only asks (isCompareMode/getCompareId) or resets (resetCompare), so they can't desync. Reads the design collection read-only via injected getters; never touches the save/concurrency baseline. Tested by links-compare.test.mjs
...
│   ├── visual.spec.js      ← visual-regression baselines (Section B/F / F-VIS, v17.73): runs ONLY via `npm run test:visual` under `playwright.visual.mjs`. Clock-pinned + Firebase-stubbed + fixed-member + overlays-dismissed → deterministic pixels; fixed-viewport (not fullPage) capture; `toHaveScreenshot` against committed baselines in `e2e/visual-baselines/`. Ten baselines locking the app surfaces (calendar desktop; paycalc desktop+mobile; admin/operations desktop; settings mobile) — incl. the accepted desktop voids — AND the four static guide layouts (railcard/fip mobile, staff/paycalc-guide desktop), which pin the Section C chip-hoist + Section D guide-content edits. So a CSS/token/layout change can't silently restyle a page. EXCLUDED from the smoke run (env-sensitive pixels). Teeth-verified (a hue-shifted token trips it); stress-verified stable. **Mobile calendar (index.html @390px) is deliberately NOT baselined** — its fractional 7-column grid rasterises with run-to-run sub-pixel variation that flakes pixel-exact diffing (covered instead by the geometry-based responsive/calendar specs). Regenerate with `npm run test:visual -- --update-snapshots`
...
npm run test:hygiene  # sw-asset-check, import-graph, links-design, admin-rangepicker, client-errors, overlay(+history), usage-stats, perf-stats, surname-parity, payday-cutoff-parity, storage-rules-static, storage-utils, auth-identity, auth-state-core, auth-state, auth-policy, sw-register, sw-internals, csp-hygiene, guide-sources, guide-colour-parity, links-analysis, links-compare, paycalc-format
...
| Paycalc desktop workspace + sticky bar (`#stickyTotal`) | ≥1024px: three-column grid — Hours + Settings span the two work columns; a col-3 sidebar (`.pc-side`) stacks the result + the four occasional cards (v16.67, owner-approved). The result is **not sticky**; the `#stickyTotal` fixed bottom bar keeps the take-home £ visible on all widths. Full layout rules (grid rows, mobile `display:contents`, phantom-gap warning): `.claude/rules/paycalc.md`. |
...
| **Annual leave cannot be recorded on a base REST day (F1, v17.77)** | AL on a day the member is already off is not real leave — it must not display or consume entitlement. Before v17.77 the "Change a Shift" week grid was the one AL path that allowed it (AL Booking's `isWorkingDate` and the roster import's `restSafe` both already excluded rest days), so it showed 🏖️ over a day off AND wrongly deducted a day of allowance. Now in lockstep, five layers (mirroring the Sunday rule; do not remove any): (1) `admin-overrides.js` week grid disables the **AL pill** on base-rest-day rows (`else if (isRestShift(baseShift))` after the Sunday block — only AL; Shift/RDW and **Other/training are still allowed on a rest day**, i.e. a rest-day `TRG RDW`); (2) the bulk-apply bar skips base-rest rows (`row.dataset.baseIsRd==='1'`) when AL is the active type; (3) the save collector in `admin-app.js` blocks `annual_leave` on a `data-baseIsRd==='1'` row with a row-error; (4) **entitlement** — the over-entitlement count excludes base-rest-day AL from BOTH existing and new dates (`isBaseRestDay` via `getBaseShift`), so legacy rest-day AL stops inflating the total; (5) **display** — `isOverrideDisplaySuppressed` suppresses `annual_leave` on a rest-day base (matching `sick`), so any legacy rest-day AL renders as the rest day it should be. Only `annual_leave` and `sick` are rest-day-suppressed; `other` is Sunday-only (rest-day training is valid). |
