...
├── overlay.js              ← shared overlay helpers: lockBodyScroll, unlockBodyScroll, _pushOverlayState, _clearOverlayHistory, dismissOverlay, trapFocus, initCardCollapse. Singleton popstate listener. Imported by all six app pages and nav-panel.js (v11.40)
...
| Links page — design and save model (v12.09) | **Card order:** Grid (primary object) → Coverage (analysis of the grid) → Initialise (utility, collapsed by default). This order is intentional — do not promote Initialise or Coverage above the grid. **Firestore document:** `linkDesigns/combined-28` holds `{ patterns, meta, updatedAt, updatedBy }`. `patterns` is `{ "1"–"28": { sun, mon, tue, wed, thu, fri, sat } }` (each value a shift string or `"RD"`). `meta` is `{ "1"–"28": { staffName: string, isFixed: boolean } }`. Both are saved in a single `setDoc()` — there is intentionally one save action, not separate saves for shifts and staff names. **Single dirty flag:** `dirty = true` whenever any shift cell or any staff name input changes. One `linksSaveBtn` / `updateSaveBtn()` / `saveChanges()` — never split these back into separate save flows for shifts vs. staff. **Unsaved-changes guard:** `beforeunload` event + explicit `confirm()` on sign-out, back-to-calendar navigation, and (v12.37) a capture-phase click guard on all nav-drawer links (mobile browsers suppress `beforeunload` dialogs). **Inline staff name editing:** The `Staff` column in the grid contains a borderless `<input class="staff-name-input-inline">` for every non-fixed row (fixed = line 28, C. Reen). Placeholder text shows the line name ("Line 1", "Line 2" etc.) so unassigned lines are identifiable at a glance. Inputs are transparent/borderless at rest; a navy bottom-border appears on focus. On `input` event the handler updates `design.meta[pos].staffName`, sets `dirty`, and (v12.37) toggles the row's `row-vacant` / `row-normal` classes live so typing isn't dimmed by the vacant-row opacity. Grid events are **delegated** on `#linksGridBodyRows` (one click + one input listener, wired once) — do NOT call `renderGrid()` from inside `saveChanges()` as it destroys focus. Name inputs carry `list="staffNamesList"` — a datalist of current `teamMembers` for typo-free assignment (free text still allowed). **Last-saved display:** `#linksLastSaved` shows "Last saved by X at HH:MM" after a write; also populated from `data.updatedAt` / `data.updatedBy` on load. **Coverage chart:** `calcCoverage()` counts `{ early, late, spare, night, rd }` per day across all 28 positions. **CEAs do not work night shifts (confirmed by Gareth June 2026)** — night times are never offered in the cell dropdown and `normaliseCustomShift()` rejects starts between 21:00 and 03:59; do not re-add a Night option. The `night` bucket in `calcCoverage` is defensive only (legacy/imported data): it counts toward `worked` and renders a bar segment + `N:` footer count if ever non-zero, but has no legend entry. Bars are 96px; `.cov-bar-ref-line` is positioned `bottom: 50%` to mark the 14/28 midpoint. Zero-coverage days (`worked === 0`) get `.cov-count-warn` / `.cov-day-label-warn` (red text). **Line numbering:** Lines 1–27 are the main rotating link (20 CEA + 2 BL + 5 vacant). Line 28 is C. Reen's fixed link — `FIXED_POS = 28`, `VACANT_FROM = 23` (vacants are lines 23–27). A `<tr class="fixed-link-separator">` row is inserted before line 28 to visually separate her as additional cover. **Initialise:** `buildDefaultDesign()` — lines 1–20 from the CEA main 20-week link (one entry per roster week), 21–22 from the BL link, 23–27 vacant (all RD), 28 fixed (C. Reen). **Staff names are intentionally left blank** for all lines except 28 (C. Reen) — who goes on which line is a separate decision made after the link patterns are agreed. After calling `buildDefaultDesign()`, `dirty` is set and `renderGrid()` + `updateSaveBtn()` are called; the save itself is not automatic. **Migration:** `loadDesign()` checks if old Firestore data has C. Reen at position 23 (pre-v12.09) and swaps it to 28 automatically. **Position numbering:** always `String(pos)` as the key (`"1"`–`"28"`), never `Number`. **Shift options:** `EARLY_SHIFTS`, `LATE_SHIFTS`, `NIGHT_SHIFTS` are derived dynamically from `weeklyRoster` + `bilingualRoster` at module load — do not replace with a static list. Early and Late groups appear in the cell dropdown (never Night — see above), plus a **Custom time…** option (v12.37) — `prompt()`, validated/padded by `normaliseCustomShift()` to `HH:MM-HH:MM` — so designers can pattern shifts that don't exist in today's rosters. **Concurrency & load safety (v12.37):** `saveChanges()` re-reads the doc before writing and compares `updatedAt` against `loadedUpdatedAt` (captured at load / refreshed after each save); on mismatch a `confirm()` names who saved and when before overwriting — `setDoc` replaces the whole document, so this is the only thing standing between two designers and silent data loss. A failed Firestore read sets `loadFailed` — the empty state then shows an error and hides Initialise (seeding over a load *failure* would invite overwriting a real saved design). **Print (v12.37):** links.html is the one sub-page whose content is a paper artefact — `@media print` renders the grid + coverage on A4 landscape (compact rows, sticky flattened, vacant-row fade lifted). Do not revert to the "nothing to print" stub. **Sticky day headers (v12.37):** at ≥1024px `.links-grid-wrapper` drops `overflow-x` and `.card` uses `overflow: clip` (not `hidden`) so the `thead` sticks to the viewport — both are load-bearing; reintroducing `overflow-x: auto` or `overflow: hidden` silently kills the sticky header. |
