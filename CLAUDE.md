...
| **AL on a rest day is ASKED about, never defaulted (v23.75)** | Leave on a base-roster rest day costs no entitlement — that is what stops a stray booking and a declined RDW charging somebody a day — **unless the member SWAPPED working days onto it**, which only the person booking knows. So both booking paths ASK, per affected day, and refuse to save unanswered; the answer is recorded as `replacedType: 'shift'`, which the rules already permit, so this needs no schema or rules change. **Ask only when a day is affected, and never default** — the two halves are the owner's, and a default is how this went wrong: the AL card silently skipped rest days while the week grid silently wrote them costing nothing, and three of one member's days went missing until the depot's workbook disagreed. Never wire it to the Absence card. Reasoning: `al-swapped-days.js`. |
