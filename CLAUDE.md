...
└── functions/
    ...
    ├── roster-members.json       ← generated from roster-data.js — do NOT hand-edit; run `npm run generate:roster-members`. Holds the AI-parsing name lists (cea/ces/dispatcher), the B4 server-owned auth lists (`activeMembers` + `roles.admin`/`manager`/`designer`) that setupRosterAuth trusts instead of the client payload, and `overtimeRoster` — who EXISTS, with each member's `hidden`/`managerOnly` flags, because who is ASKED is an audience decision and belongs beside the other audiences in `overtime-core.js`. CI-locked by sw-asset-check.test.mjs
    └── package.json
```
