...
| Documentation milestone | `24.30` — the 0.10 checkpoint these docs are written against. **This row is deliberately NOT the current version**, and used to be: it read `23.23` while the app shipped 23.24, then 23.25, then 23.26 — a figure that rots on every release and that nobody bumps, because bumping it is not part of shipping. The exact version lives in `roster-data.js`, where `APP_VERSION` is authoritative and is the only place a reader should look for it. What the guard actually requires here is the MILESTONE, so that is what this row now states. The version stamp in **every** doc (this file, AI_MAP, OPERATIONS_REFERENCE, KNOWN_LIMITATIONS, ROADMAP) is enforced against the latest 0.10 milestone by `sw-asset-check.test.mjs` and `githooks/pre-commit` — a bump crossing a 0.10 line fails until each doc is reviewed and re-stamped. |
...
| | `https://europe-west2-myb-roster.cloudfunctions.net/getDocumentUrl` (v24.16 — a SHORT-LIVED URL for the latest Huddle / Circular / Newsletter, replacing the permanent bearer URL. The caller names a KIND, never a path; the door mirrors `firestore.rules`. **Needs `roles/iam.serviceAccountTokenCreator` on the runtime service account, granted to ITSELF** — without it every call 503s and the client falls back to the stored URL, which is exactly the pre-v24.19 behaviour. **The three opening surfaces call it since v24.19** (nav drawer, `calendar-doc-viewer.js`, `calendar-huddle-viewer.js`), minting BEFORE the click — awaiting inside a handler spends the user gesture and gets `window.open` pop-up-blocked. AUTH_PLAN.md §5) |
...
| | `https://europe-west2-myb-roster.cloudfunctions.net/requestPasswordReset` (one of the app's TWO public unauthenticated endpoints, with `unlockCalendarViewer` — a locked-out member asks the admin to reset their password; records a request, never resets anything. Since v18.95 it also pushes the request to the **admin's devices only** via `sendTargetedPush` — never `fanOutPush`) |
| Development branch convention | `claude/<description>-<sessionId>` — always push to this branch, never directly to `main` |

**Secrets** (Firebase Secret Manager unless stated — no workflow reads any of these; deploys authenticate by WIF):
...
| `VAPID_PRIVATE_KEY` | Web Push private key — Secret Manager. The PUBLIC key is a literal in `functions/index.js` **and** `notif.js`, and the two must match (see Change impact) |
...
# THE REAL-PAYSLIP LANE, WHICH REQUIRES THE GITIGNORED FIXTURE AND FAILS WITHOUT IT.
# `npm test` skips the payslip regression honestly when the fixture is absent — correct for a public
# lane, and it means a green run cannot tell you whether the comparison RAN. This one can: run it
# before any pay-maths release. Owner's checkout only, by design:
npm run test:payslip-local
...
npm run test:functions # Cloud Functions tests (roster-parse-helpers.test.mjs + functions-surface.test.mjs, which requires functions/index.js and pins the deploy surface, + overtime-endpoints.test.mjs + auth-endpoints.test.mjs + push-transport.test.mjs + documents-endpoints.test.mjs + index-endpoints.test.mjs, which drives the two handlers that stayed in the composition root) — not part of npm test (needs functions/node_modules)
                       ...
                       # blocked (production safe, wrong end of the merge). That job switches to Node 20 only for firebase-tools auth,
...
| **When a build step earns its keep (threshold, not yet crossed)** | The no-build rule is a deliberate trade — zero toolchain, direct debuggability, no build-supply-chain — paid for by hand-maintained work a bundler does for free (the `modulepreload` lists, the SW precache, the 2-location version bump, `generate-*` codegen, the CSP `*-boot.js` shims, the pure-helper splits, the `normaliseSurname` duplication). **Revisit — do not auto-adopt — when either (a) you want real TypeScript types, or (b) a bug is traced to drift in a hand-maintained list.** Until then the trade favours no build. The full cost list, why the figures are COUNTED rather than written down, and the third input that is measured but is NOT a trigger: **DECISIONS.md → "No build step"**. |
...
| **No native `<select>` POPUP (v23.33)** | A select's list is drawn by the OS, so the app's design stops at its edge — on Android it is a full-bleed Material radio sheet, which is how a ~50-name roster arrived as fifty oversized ungrouped rows. **Every dropdown a reader READS goes through `enhanceSelect` / `initSelectSheets` (`select-sheet.js`)**; the `<select>` stays as the value holder, so consumers are untouched. The rules an ENHANCEMENT breaks without touching the call site — the `.fieldpick` and `#<id>Trigger` CSS counterparts, never `.focus()` an enhanced select, announce a selection changed IN CODE, never enhance a `hidden` one — are numbered contracts in **`select-sheet-parity.test.mjs`**, each failing with its own fix. That file also holds every native-by-decision exclusion with its reason and carries the argument; DECISIONS.md has the history, including the sign-in cascade (decided and enhanced, 9 Sep 2026). |
| **The OS draws nothing the app can draw (v23.50)** | The `<select>` rule above, generalised: where the platform draws a control, the app's design stops at its edge. Checkboxes and radios (ONE recipe in `shared.css` — recolour through `--check-fill`/`--check-border`, and never set a size or an `accent-color`), textarea resize grips, scrollbars, `::selection`, `::placeholder`, `caret-color`, autofill's repaint and the password field's OS buttons are all app-drawn now. **`title` tooltips were REMOVED, not rebuilt** — a hover-only surface is the wrong answer on a phone, so the few that explained a DISABLED state became a line in the row instead. Safari's strong-password button is LEFT: a feature, not a duplicate. `native-surface-parity.test.mjs` enforces every recipe; the reasoning is each recipe's own header, `.claude/rules/css-tokens.md` → Checkboxes and radios / The six the first audit missed, and DECISIONS.md → Tooltips. |
...
| "Recording for &lt;name&gt;" on the AL and Absence cards (v22.45) | `.card-member-for` at the TOP OF EACH CARD BODY, filled by `_syncMemberFor` in `admin-app.js` from the same sites that sync the two member displays. **Managers and admin only** — they are the only identities that can point these cards at somebody else, and for a self-service member the row would state the one fact they already know. **Three things not to "tidy"** — it is not in the `.card-header-actions` cluster, it is outside `#alBanner`, and its `[hidden]` rule is load-bearing — each with its reason beside the `.card-member-for` block in `admin.css`. |
| Type pills in admin — single source of truth (v13.48) | `PILL_TYPES` in `admin-shift-types.js` is the ONE authoritative list, and neither pill row may restate it: `renderWeekGrid()` (admin-week-editor.js) generates the per-row pills, `admin-app.js` the bulk bar — **minus `other`**, deliberately (its flavours need per-row sub-controls a bulk bar cannot host; the rationale is beside the filter). Order: AL · Shift · RDW · Absent · Rest Day · Other — 6 top pills since Spare moved into the Other submenu at v15.57. Other reveals per-row sub-controls (flavour chips + a pre-ticked-on-rest-day RDW tick + OPTIONAL times, `timesOptional: true`) and the save collector composes `FLAVOUR[" RDW"][" HH:MM-HH:MM"]`. **Spare is special-cased**: it stays its own `spare_shift`/'SPARE' type (📋, not worked), so picking it writes a `spare_shift` and hides the tick and times. Why the table is its own module: `admin-shift-types.js`'s header. |
...
| Sync chip state machine in `calendar-initial-fetch.js` | hidden → (800ms) → "↻ Updating…" → silent remove on success, or "⚠ Couldn't update" (stays, 10s timeout). "✓ Up to date" removed (v10.19) — noise. Never show raw errors to staff. Hidden while the grid shows its OWN loading/failed panel (one message, not two); kept beside a failed panel that has no Try again, where it is the only retry (v24.30). |
...
| One-off notification prompt (`#notifPrompt`) | Appears once per device between `</nav>` and the swipe hint / calendar. Both Enable and × set `myb_notif_prompt_done`. Do not move below the calendar. |
...
| 3-digit time input auto-correction in `admin-week-editor.js` | On blur, if length is 3 and `parseInt(raw.slice(0,2)) > 23`, prepend `'0'`. Without this, `"630"` produced `"63:0"`. |
| Range picker clear button (`.rp-clear`) | Resets both `from` and `to` dates. Built into `buildRangePicker()` in `admin-rangepicker.js`. |
| **Sundays are non-contracted — AL and Absent cannot be recorded on Sundays** | Sundays are uncontracted for all grades. The forbidden write types are declared ONCE, as `SUNDAY_FORBIDDEN_TYPES` in `override-utils.js` — `annual_leave`, `sick`, `other` and plain `shift` (a worked Sunday is `rdw`, deliberately absent). **Do not restate the list in prose or in a new call site**; consult it, or be pinned to it by `override-utils.test.mjs`. **SIX enforcement layers, none removable alone** — they are numbered in the code comments, so APPEND rather than renumber: (1) week-grid disables the AL/Absent pills on Sunday rows and the Shift pill too; (2) bulk-apply silently skips Sunday rows; (3) `recordRangeOverrides` filters Sundays out of `workingDates`; (4) roster upload normalises a Sunday `AL`/`SICK` to `RD`; (5) display — `isOverrideDisplaySuppressed` hides a leave, absence or Other override on a Sunday, so legacy data never renders; (6) the single-row save in `admin-app.js`, which answers for all four types with three different outcomes (`shift` → promoted to `rdw`; `other` → refused unless Spare; `annual_leave`/`sick` → refused) and so cannot loop over the list — pinned to it by a test instead. A worked Sunday time is always RDW, never AL/Absent. |
| **AL on a rest day is ASKED about, never defaulted (v23.75)** | Leave on a base-roster rest day costs no entitlement — **unless the member SWAPPED working days onto it**, which only the person booking knows. So both booking paths ASK, per affected day, and refuse to save unanswered; a declared swap is recorded as `replacedType: 'shift'`, which the rules already permit. **Ask only when a day is affected, and never default** — both halves are the owner's, and a silent default lost three of one member's days. **"Rest day — free" means NOTHING IS WRITTEN, on both surfaces** (v23.88), and the save receipt NAMES the days it left alone — a save may not do nothing silently. **A day that already HELD leave is named differently** (v23.93): answering never deletes, so the record stands and the Calendar still shows it; reporting it like an empty day contradicted the grid. **The answer must reach every layer, not just the write** (v23.79) — one projection feeds preview, warning and save. Never wire any of it to the Absence card. Reasoning: `al-swapped-days.js` (which days are asked), `admin-al-projection.js` (what the answers mean, and why the grid reads `answeredFree` rather than `skipped`) and `admin-al-week-save.js` (what one save does with a batch of them). |
...
| **Calendar access: a named session OR the staff PIN (v20.12)** | `overrides` reads require a member `name` claim or the shared `calendarViewer` capability. **Sign-in card first, PIN one tap behind** — an order, not a grant. **Anonymous grants nothing**, which is the substance: the Calendar used to sign every visitor in anonymously, so `request.auth != null` would re-admit exactly the people it closes out — ask for a CLAIM, never a session. **CLOSED 26 Aug 2026 — the PIN is the real boundary now**, so `CALENDAR_PIN_ACCESS: false` is no longer a rollback; reverting the rules push is. **The invariants are NOT restated here**, because two contracts own them and both require linking rather than repeating: `CALENDAR_DATA.md` 3/4/11/12/13 and `AUTH_AND_SESSIONS.md` 7–10, 14 and **18** (the PIN is a barrier, not authentication — and what no surface may say about it). The owner-decided PROVISIONAL PAINT exception was retired 26 Sep 2026 (DECISIONS.md). Rollout, rollback, the caching caveat and the standing GCP IAM prerequisite: RECOVERY_RUNBOOK.md → "The Calendar PIN"; operations: OPERATIONS_REFERENCE.md. |
...
| Android Back button overlay pattern | Overlays push `history.pushState({ mybOverlay: true })` when opening, close on `popstate`. `_pushOverlayState(handler)` / `_clearOverlayHistory()` are exported once from `overlay.js` and reached through `createLightbox`. |
| Canonical lightbox lifecycle (standardised v11.50, factored into `createLightbox` v12.50) | Every `.lb-overlay` lightbox is built with **`createLightbox({ overlay, content, closeBtn, initialFocus, onOpen, onClose })` in `overlay.js`** — do NOT hand-write the lifecycle in a page module. It owns focus save/restore, the scroll lock, Android Back, Escape, the Tab trap, backdrop/close-button close and the mandatory `transitionend` fallback. Close controls are `<button class="lb-close">`, never `<span>` — not keyboard-focusable. **Three exceptions, each with its reason: AI_MAP → `overlay.js`** — the drawer's two lightboxes (they share the drawer's history entry, which is why they hand-roll it) and the huddle viewer (a full-bleed panel). Full lifecycle detail is in that same entry. |
| Nav panel on all 7 pages (v10.57; **restructured v20.06**, Reference reopened v20.09) | `nav-panel.js` injects overlay + drawer; burger `#navMenuBtn` in each page header. **THREE ZONES, ONE IDIOM EACH:** pills = go to a page · Latest = the documents you open on a shift · Reference = look something up. `NAV_PAGES` drives the pill row, `NAV_INFORMATION` the flat Workplace section, `NAV_GUIDES` the Reference submenu. **Four rules that have each been broken once** — the current page renders as an inert `aria-current` pill, NOT omitted · Settings is a pill, not a pinned link · pills are one per row · `aria-expanded` is the ONLY state for the Reference fold (no `.open` class — re-adding one re-creates the v20.09 stuck-arrow bug). **The measurements and the argument behind all four are in `nav-panel.js`'s header.** Adding a guide = one `NAV_GUIDES` entry; adding a live doc = one `links` entry in `NAV_INFORMATION`; `comingSoon: true` renders a `<button>` opening `#navComingSoonLightbox`. |
...
| Nav-panel logo = About; drawer head shows version (v11.21) | The drawer head is a `#navPanelBrand` button (logo + title + `Version {APP_VERSION}` muted text). Tapping it closes the panel (`closePanel`) and calls `onLogoClick` once that close's Back echo has landed (`whenHistorySettled`), which each page passes as `() => openAboutLightbox?.()` — opening that page's existing `#iconLightbox` (version, update status, bug report, and page-specific print/guide links). Each page exposes its scoped open fn through a module-level `let openAboutLightbox` assigned inside its About-lightbox IIFE. This replaces the header logo's old role (see header-logo back button entry). |
| Settings page — shared session, flat nav link (v11.06) | `settings.html` uses the same `AUTH_KEY` as `admin-app.js` — a user already signed in on any page arrives without seeing the login overlay. **The nav is wired by `wireNavPanel()`, NOT at module scope, and deliberately NOT for a signed-out visitor**: it is deferred to `initAuthorised()` so the drawer renders ONCE, with the signed-in identity, and the full-screen login overlay covers the burger meanwhile. Operations and Links do the same; Overtime is the one page that still wires the drawer while signed out, on purpose, and its own comment says why. A signed-out visitor is not stranded — the login overlay carries its own "← Back to roster" (`login-overlay.js`). Why it is not module scope, and what a reader restoring that would re-create: the comment above `wireNavPanel` in `settings-app.js`. Settings is a `NAV_PAGES` pill since v20.06 — see the Nav panel row. |
...
| SW offline fallback only for navigation requests (v10.15) | Only `event.request.destination === 'document'` requests get the offline HTML page. JS/CSS get `Response.error()`. Without this, `/admin-app.js` matched `'admin'` in the fallback logic and got HTML for a JS request — MIME-type error. Fallback routing chain (v11.87): `paycalc` → `paycalc.html` · `operations` → `operations.html` · `settings` → `settings.html` · `links` → `links.html` · `overtime` → `overtime.html` · `admin` → `admin.html` · otherwise → `index.html`. |
| Huddle notification → `#huddle` hash pattern | SW navigates to `#huddle`; the `hashchange` handler in `calendar-huddle-viewer.js` triggers the viewer (`calendar-doc-viewer.js` handles `#circular`/`#newsletter`). **Two viewer paths — do not unify, and do not revert the notification path to a direct `window.open`/`location.href`:** with `htmlContent` (DOCX converted server-side) it renders inline; without it (PDF, or a failed conversion) the viewer offers an in-overlay "📄 Open Huddle" button, whose click is a REAL GESTURE — a direct open on a hash-open would be pop-up-blocked or knock the PWA out of standalone. The viewer opens **only** via the `#huddle` hash, from both the drawer link and a notification tap. **The three documents are behind the PIN or a password (owner decision, 7 Sep 2026; client v23.17, rules v23.18)** — the viewers and the drawer's links consult `calendar-doc-access.js`, so a locked visitor's tap is answered with what to do and no read is issued. Full rationale: **OPERATIONS_REFERENCE.md → "Huddle notification tap behaviour"**. |
...
| **A calendar day opens the day panel — on EVERY pointer type (v23.59)** | Owner request: *"I want the day detail lightboxes like mobile on desktop."* The click handler in `calendar-renderer.js` has **no pointer branch** and must not regain one — its own comment says what it replaced and why. **The pay jump is not lost, it is one click deeper**, on the panel's `#dayDetailPayBtn`, which `personalActionsAllowed` already gates. **Three things that go with it and are easy to undo** — the hover tooltip stays as the preview and `body.lb-open #calTooltip` takes it down, a fine pointer gets `cursor: pointer` on a real day cell, and the KEYBOARD is deliberately unchanged (Enter still jumps from a pay cell and falls through to the panel when refused, v23.07). Each is argued beside its rule in `index.css`, including which parts were MEASURED rather than assumed; `e2e/calendar.spec.js` pins the behaviour rather than the mechanism. |
...
| SW `new Request(url)` fetch pattern (v10.16) | `new Request(event.request.url, { cache: 'no-cache', ... })` (no-cache, so the browser can revalidate with a 304) instead of passing opts to an existing Request. Passing opts alongside a Request doesn't reliably override cache mode on older Safari/Chromium. |
| `initErrorReporter()` call pattern (v13.78) | It writes to Firestore `clientErrors`, so a valid auth token is required. **Never call it bare** — without an auth context every write is silently rejected by the rules, so the error log looks HEALTHY because it is broken. **Three canonical call sites**, differing because the pages establish identity differently: `calendar-app.js` (wait for persistence, reconcile an expired identity, sign in anonymously only if no named user remains — the ordering PRESERVES a valid identity rather than racing it) · the pages with `sessionReady` (`admin`, `settings`, `operations`, `links`, `overtime`) · `paycalc-app.js`, which has none. The exact shapes: the header of **`error-reporter.js`**; the rule: `AUTH_AND_SESSIONS.md` invariant 13. |
...
> **⚠️ This file is WORLD-READABLE.** The table lives in `roster-member-data.js` (re-exported by `roster-data.js`); both are served from both origins with no session,
...
> work email) already does. `public-data-classification.test.mjs` fails on an unclassified field. **A PATTERN IS PUBLIC; A PAYSLIP IS NOT** — `payroll-anonymity.test.mjs` fails when a roster name sits on the same line as a payroll term anywhere in the tracked tree, because the Pages mirror serves the test files themselves (measured: 200 there, 404 on Firebase). Keep the arithmetic, drop the attribution.
...
- **"account recovery"** is now the term for what a saved work email is FOR (v22.37, owner decision — this row previously banned the word "recovery" outright and mandated "self-service password reset (in a future update)"). The old phrasing was accurate and read like a product roadmap; the replacement separates the two questions a member actually has. **Why you are giving us this:** "Save your Chiltern work email for account recovery." **What works today:** "Self-service recovery isn't available yet. For now, ask the admin if you forget your password." Still never "coming soon". The future email route is SECURITY_RELEASE_PLAN.md track C2 (the original stage plan is in ROADMAP_HISTORY.md). **Note (v18.63):** a password *reset* now exists **today** via the admin (Operations → Account status → Reset) and staff can set their own password (Settings → Password), so do NOT say a password reset is flatly "not available" — only the *email self-service* route is future. Present-day recovery copy is "To reset a password today, contact the admin."
- **App name "Marylebone Roster"** — the on-screen name everywhere (incl. bug-report `appLabel` as "Marylebone Roster — <Page>"). **Never "MYB" for the APP or one of its tools** — not "MYB Roster", not "MYB Pay Calculator", not "MYB member". As the app name it survives only in the iOS home-screen `apple-mobile-web-app-title` meta, the manifest's `short_name` (exempt by key in `app-name-parity.test.mjs`) and comments.
...
**Everything else about the import is `.claude/rules/roster-import.md`**, which `paths:`-globs the
parser, the alignment check, the review states and the cell controls, so it loads whenever one of
them is opened and costs nothing when it is not. It holds the review's unreadable-cell control, the
`UNKNOWN|<raw>` sentinel, the day-drift defence and **why its three "layers" are not three
witnesses**, the geometry gate that is the one independent witness, and the rules for what a blank
cell means. **Read it before changing anything in the import** — every rule in it is there because a
week was written onto the wrong days, or nearly was.
...
**Example payslips for testing:** the pay-maths regression net is thirteen REAL payslips, kept OUT of the repository since v23.71 — the full reasoning (why a gitignore and not an ignore list, why the example fixture is never imported, why ABSENT is a skip but BROKEN is a failure, and what `npm run test:payslip-local`'s green actually proves) is **`.claude/rules/paycalc.md` → Example payslips**, which loads whenever a paycalc file is open and costs nothing when it is not.
