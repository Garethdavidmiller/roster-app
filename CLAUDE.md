...
├── railcard-guide.js       ← JS for railcard-guide.html: print, chip-bar navigation. The PAGE was rebuilt for the Marylebone gateline at v20.38: it separates the RAILCARD's discount rule from the TICKET's validity from CHILTERN's revenue procedure, groups the cards by the KIND of morning rule (none / minimum fare / fixed cutoff / follows-the-ticket), and carries a directional Chiltern Off-Peak panel in place of the invented `~10:00` shortcuts. **The Network AREA is `Draft` and must stay that way** — the boundary is a filled polygon on a map whose labels are vector artwork, so it cannot be read here; v20.38 briefly "corrected" it to reach Birmingham on a bad inference from the map's station index (which also lists Bristol and Exeter) and had to unwind. Neither certainty may be stated: one refuses valid tickets, the other accepts invalid ones. Full design + the source audit: `.claude/rules/guide-pages.md`
