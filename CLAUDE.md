...
| Current app version | `16.60` (latest 0.10 milestone; exact value in `roster-data.js` — `APP_VERSION` is authoritative). The version stamp in **every** doc (this file, AI_MAP, OPERATIONS_REFERENCE, KNOWN_LIMITATIONS, ROADMAP) is enforced against the latest 0.10 milestone by `sw-asset-check.test.mjs` and `githooks/pre-commit` — a bump crossing a 0.10 line fails until each doc is reviewed and re-stamped. |
...
| Paycalc desktop workspace + sticky bar (`#stickyTotal`) | ≥1024px: THREE-column grid — Hours + Settings span the two wide work columns, four short cards 2-up below, and `.result-card` is a dedicated col-3 sticky live-estimate RAIL (`position:sticky`, grid-row 4/8, ALWAYS visible). The rail is overlap-safe ONLY because nothing else is in col 3; do NOT add a card to col 3 or make the result sticky while it shares a column (v16.14 — verified twice a sticky result beside content overlaps it). `#stickyTotal` fixed bottom bar on ALL widths (desktop capped to `--content-max-width`, centred) backstops the footer rows below the rail. `IntersectionObserver` + `body.sticky-active`. See `.claude/rules/paycalc.md`. |
