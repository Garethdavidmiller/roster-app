...
| Current app version | `12.21` (check `roster-data.js` — `APP_VERSION` is the authoritative source) |
...
├── nav-panel.js            ← shared slide-out navigation drawer: initNavPanel(opts), NAV_PAGES config, NAV_INFORMATION config, NAV_GUIDES collapsible submenu, brand logo→About (onLogoClick) + version, footer notification bell. Imported by app.js, admin-app.js, paycalc.js, operations-app.js, settings-app.js
...
├── overlay.js              ← shared overlay helpers: lockBodyScroll, unlockBodyScroll, _pushOverlayState, _clearOverlayHistory, dismissOverlay. Singleton popstate listener. Imported by app.js, admin-app.js, paycalc.js, operations-app.js, settings-app.js, nav-panel.js (v11.40)
...
├── settings-app.js         ← coordinator for settings.html: session check (shared AUTH_KEY), login overlay, initAvatarCard, initHuddleNotifications, cultural calendar init, initNavPanel (v11.06)
├── settings-avatar.js      ← Profile photo card (initAvatarCard → settings.html): choose/preview/save/remove, client-side square-crop + JPEG compression to ~256px, localStorage cache + myb:avatar-changed event. Imports uploadAvatar/deleteAvatar/fetchAvatarUrl from firebase-client.js, paintAvatar/avatarCacheKey from avatar.js (v12.12)
├── avatar.js               ← shared avatar helpers: avatarCacheKey + paintAvatar(el, url, name) DOM painter (photo with onerror→initials fallback). Imported by settings-avatar.js and nav-panel.js so the settings preview and footer badge render identically (v12.12)
...
├── links.html              ← 28-position link design workspace (v12.07); visible only to CONFIG.LINKS_DESIGNERS
├── links.css               ← all CSS for links.html — grid table, cell colours, coverage chart (with reference line), inline staff name input (v12.07)
├── links-app.js            ← coordinator for links.html: auth guard, Firestore load/save for linkDesigns/combined-28, grid render, inline staff assignment, unified save, coverage analysis, init-from-rosters (v12.07)
...
| Profile photo / avatar (v12.12; reposition editor v12.19) | **⚠️ NON-VITAL feature — revert candidate.** A "nice to have" (a member's name becomes a photo in a menu). A decision may be taken later to remove it, simplify it (auto centre-crop or slider-only — dropping the high-risk interactive editor), or revert fully. The **exact revert checklist, footprint (~865 lines / ~700 JS), and the honest "the editor is gold-plated for a 26px badge" assessment live in ROADMAP.md → "Profile photo / avatar"**. Keep the display/storage/sync layer if trimming; the interactive editor is the only over-built part. — A member's optional profile photo. **Upload UI** is the Profile card on settings.html (`settings-avatar.js`, `initAvatarCard`). Flow is deliberately simple ("for dummies"): Choose photo → **reposition** → Save; or Remove to clear. **Reposition editor (v12.19):** an inline square `<canvas>` editor (`#avatarEditor`, hidden until a photo is chosen) lets the user drag to pan and zoom (slider · pinch · wheel · arrow keys) to frame the shot before saving. One geometry state `{ z, px, py }` driven through a single `setZoom(z, anchorX, anchorY)` (zoom-about-a-point) + `clampPos()` (keeps the image covering the square — no gaps). Both the live preview and the exported ~256px JPEG are rendered from the SAME orientation-corrected `ImageBitmap` with identical math (`coverScale = max(V/Wn, V/Hn)`), so what you see is what is saved — this avoids the EXIF-mismatch class of bugs. `devicePixelRatio` only affects the on-screen backing store, never the natural-pixel export. Editor "Save photo" exports → uploads directly (no separate pending row — removed at v12.19); "Cancel" discards and restores the saved photo. `ImageBitmap.close()` + `cancelAnimationFrame` on close/cancel/re-pick; `touch-action:none` on the canvas so page scroll/zoom never fights the gesture. A circle ring overlay (`.avatar-editor-ring`) shows the inscribed circle the footer badge clips to. JPEG-compress to ~256px happens **in the browser** (canvas). **Storage:** image at `avatars/<slug>.jpg` (Storage), pointer doc at `memberAvatars/<memberName>` (Firestore, open-read). All Storage/Firestore writes `await window._mybSession` first so Firebase Auth is live (same as the cultural-calendar write). **Display:** a 26px circular badge in the nav-panel footer (`#navPanelAvatar`, before the name — it replaces the old `👤` glyph), and an 88px preview on settings. **Fallback when no photo:** initials on a stable per-name colour, via the shared `avatarInitials(name)` / `avatarHue(name)` pure helpers in `roster-data.js`. The DOM painter + cache key live in **`avatar.js`** (`paintAvatar(el, url, name)`, `avatarCacheKey(name)`), imported by BOTH `settings-avatar.js` (88px preview) and `nav-panel.js` (footer badge) so the two render identically — do not re-duplicate them. `paintAvatar(el, url, name, onError?)` attaches an `img.onerror` that falls back gracefully on a load failure (rotated/deleted Storage token, offline) instead of a broken-image glyph: to **initials** by default, or — when the optional `onError` callback is passed — to whatever it paints. The About lightbox passes an `onError` that **restores the app logo** (a broken photo there must not become an initials block). `fetchAvatarUrl` **rejects** on a read error and resolves `null` only for a genuine no-avatar read, so callers clear the cache on real removal but keep it (catch) on a transient error. **Cross-page/-device sync (three layers):** (1) URL cached in `localStorage('myb_avatar_<name>')` for instant paint; (2) nav-panel refreshes from Firestore on load (covers a photo set on another device) and writes the result back to the cache so it converges; (3) live updates after a save/remove — a `myb:avatar-changed` CustomEvent for the **same tab**, plus a `window` `storage` event listener for **other tabs** of the same browser (the storage event never fires in the writing tab, so both are needed). A module-level `_avatarSettled` flag in nav-panel makes any of these win over the slower init-time fetch. Avatar images are cross-origin (Storage) so the SW never touches them (it only handles same-origin). The CSP already allows `img-src`/`connect-src` for `firebasestorage.googleapis.com` — no CSP change. Per-member write isolation is NOT enforced (the `name` JWT claim is suspended — see KNOWN_LIMITATIONS task #2); writes are auth-only, matching the rest of the app. |
...
| Links page — design and save model (v12.09) | **Card order:** Grid (primary object) → Coverage (analysis of the grid) → Initialise (utility, collapsed by default). This order is intentional — do not promote Initialise or Coverage above the grid. **Firestore document:** `linkDesigns/combined-28` holds `{ patterns, meta, updatedAt, updatedBy }`. `patterns` is `{ "1"–"28": { sun, mon, tue, wed, thu, fri, sat } }` (each value a shift string or `"RD"`). `meta` is `{ "1"–"28": { staffName: string, isFixed: boolean } }`. Both are saved in a single `setDoc()` — there is intentionally one save action, not separate saves for shifts and staff names. **Single dirty flag:** `dirty = true` whenever any shift cell or any staff name input changes. One `linksSaveBtn` / `updateSaveBtn()` / `saveChanges()` — never split these back into separate save flows for shifts vs. staff. **Unsaved-changes guard:** `beforeunload` event + explicit `confirm()` on sign-out and back-to-calendar navigation (mobile browsers suppress `beforeunload` dialogs). **Inline staff name editing:** The `Staff` column in the grid contains a borderless `<input class="staff-name-input-inline">` for every non-fixed row (fixed = line 28, C. Reen). Placeholder text shows the line name ("Line 1", "Line 2" etc.) so unassigned lines are identifiable at a glance. Inputs are transparent/borderless at rest; a navy bottom-border appears on focus. On `input` event the handler updates `design.meta[pos].staffName` and sets `dirty`. After a successful save, row classes (`row-vacant` / `row-normal`) are updated in-place via `classList.toggle()` — do NOT call `renderGrid()` from inside `saveChanges()` as it destroys focus. **Last-saved display:** `#linksLastSaved` shows "Last saved by X at HH:MM" after a write; also populated from `data.updatedAt` / `data.updatedBy` on load. **Coverage chart:** `calcCoverage()` counts `{ early, late, spare, night, rd }` per day across all 28 positions. Night shifts (`classifyShift` returns `'night'`) must have their own bucket — adding them to `rd` is a bug (was fixed at v12.07). Bars are 96px; `.cov-bar-ref-line` is positioned `bottom: 50%` to mark the 14/28 midpoint. Zero-coverage days (`worked === 0`) get `.cov-count-warn` / `.cov-day-label-warn` (red text). **Line numbering:** Lines 1–27 are the main rotating link (20 CEA + 2 BL + 5 vacant). Line 28 is C. Reen's fixed link — `FIXED_POS = 28`, `VACANT_FROM = 23` (vacants are lines 23–27). A `<tr class="fixed-link-separator">` row is inserted before line 28 to visually separate her as additional cover. **Initialise:** `buildDefaultDesign()` — lines 1–20 from the CEA main 20-week link (one entry per roster week), 21–22 from the BL link, 23–27 vacant (all RD), 28 fixed (C. Reen). **Staff names are intentionally left blank** for all lines except 28 (C. Reen) — who goes on which line is a separate decision made after the link patterns are agreed. After calling `buildDefaultDesign()`, `dirty` is set and `renderGrid()` + `updateSaveBtn()` are called; the save itself is not automatic. **Migration:** `loadDesign()` checks if old Firestore data has C. Reen at position 23 (pre-v12.09) and swaps it to 28 automatically. **Position numbering:** always `String(pos)` as the key (`"1"`–`"28"`), never `Number`. **Shift options:** `EARLY_SHIFTS`, `LATE_SHIFTS`, `NIGHT_SHIFTS` are derived dynamically from `weeklyRoster` + `bilingualRoster` at module load — do not replace with a static list. |
...
**memberAvatars** (doc ID = memberName, profile photos — v12.12)
```
avatarUrl    Public tokenised Firebase Storage download URL of the photo
updatedAt    Firestore server timestamp
updatedBy    memberName who set it
```
Open-read (like `huddles`/`overrides`) so the nav footer can render the avatar on
index.html, which has no Firebase Auth session. The image file lives at
`avatars/<slug>.jpg` in Storage (slug = memberName with non-alphanumerics → `_`).
Writes require auth; image-only, ≤2 MB (see `storage.rules`).
