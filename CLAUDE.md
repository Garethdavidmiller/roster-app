...
| One-time notice lightboxes — pattern (v12.33, extended v13.17) | A one-time notice lightbox announces a feature or prompts an action once per device, then archives itself to App Notices. **Recipe:** (1) Add `lb-overlay` markup to the page HTML — one emoji div, `lightbox-app-name` title, `lightbox-badge` section label, one or two `notice-lb-desc` paragraphs, optional `lightbox-bug-link` for a CTA. Card ID-specific CSS goes in the page CSS file (flex column, center, dark glass: `background: rgba(255,255,255,0.08)`, `border: 1px solid rgba(255,255,255,0.15)`, `border-radius: 24px`). (2) In the page JS module, add a guarded IIFE at the bottom: `import { createLightbox } from './overlay.js'` and `import { archiveNotice } from './nav-panel.js'`. Gate on `lsGet(NOTICE_KEY)`. The `onClose` callback calls `lsSet(NOTICE_KEY, '1')` and `archiveNotice({ id, title, section, date, body })`. If the notice has a CTA link that navigates away, add a `{ once: true }` click listener that calls the same `archiveAndMark` function before the browser navigates — otherwise the notice reappears on return. (3) `archiveNotice` deduplicates by `id`, so calling it from both paths is safe. **IDs** must be unique and stable (e.g. `'work-email-2026-06'`). **`section`** should match the page area the notice relates to (e.g. `'Settings'`, `'Admin'`). **`date`** is a display string (e.g. `'June 2026'`). **Do not use `createLightbox` in the two nav-panel lightboxes** — they are exempt (see Canonical lightbox lifecycle entry). Existing instances: `#betaLightbox` (links.html, key `myb_links_beta_seen`) · `#workEmailNoticeLightbox` (admin.html, key `myb_work_email_notice_seen`, v13.17). |
