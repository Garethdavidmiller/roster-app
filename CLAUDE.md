...
| **AL on a rest day is ASKED about, never defaulted (v23.75)** | Leave on a base-roster rest day costs no entitlement — that is what stops a stray booking and a declined RDW charging somebody a day — **unless the member SWAPPED working days onto it**, which only the person booking knows. So both booking paths ASK, per affected day, and refuse to save unanswered; a declared swap is recorded as `replacedType: 'shift'`, which the rules already permit. **Ask only when a day is affected, and never default** — both halves are the owner's, and a silent default is what lost three of one member's days. **"Rest day — free" means NOTHING IS WRITTEN, on both surfaces** (v23.88), and the save receipt NAMES the days it left alone — a save may not do nothing silently. **The answer must reach every layer, not just the write** (v23.79): preview, over-entitlement warning and save all read ONE projection. Never wire any of it to the Absence card. Reasoning: `al-swapped-days.js` (which days are asked), `admin-al-projection.js` (what the answers mean, and why the grid reads `answeredFree` rather than `skipped`) and `admin-al-week-save.js` (what one save does with a batch of them). |
