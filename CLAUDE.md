...
├── overtime-format.js      ← the PURE client side of Overtime — how a window, a phase and a day's answer are put into words, into one of three chip colours (`answerTone`) and into "your shift has changed since you answered" (`answerAnchorStale` — **unknown roster is not a changed roster**), plus `clockOffset` (round-trip corrected) and `submitDisposition`. **The client never refuses a submission near a deadline**: inside the grace band it sends and lets the server decide, because a client that refuses has denied somebody who was in time. Tested by overtime-format.test.mjs
