...
├── OVERTIME_PLAN.md ← the design for a SIXTH app page — collecting staff overtime availability for the roster clerk. **NOT STARTED**; three prerequisites stand in front of it. Carries the invariants, the three-rung beta (one submitter → a small pilot → everyone, because a 1-of-1 count exercises none of the presentation the feature exists for), the participant-snapshot security model, and the missing-window problem — the silent failure a weekly-window system has by construction. Not version-stamped; not a runtime asset.
...
├── functions-surface.test.mjs ← pins functions/index.js's eleven exports (the DEPLOY surface — a re-export dropped in a refactor is a function DELETED on the next deploy, and nothing else fails first). Runs in test:functions, not npm test: it requires firebase-admin/functions from functions/node_modules
...
npm run test:functions # Cloud Functions tests (roster-parse-helpers.test.mjs + functions-surface.test.mjs, which requires functions/index.js and pins the eleven-function deploy surface) — not part of npm test (needs functions/node_modules)
