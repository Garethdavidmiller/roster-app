...
| Current app version | `19.50` (latest 0.10 milestone; exact value in `roster-data.js` — `APP_VERSION` is authoritative). The version stamp in **every** doc (this file, AI_MAP, OPERATIONS_REFERENCE, KNOWN_LIMITATIONS, ROADMAP) is enforced against the latest 0.10 milestone by `sw-asset-check.test.mjs` and `githooks/pre-commit` — a bump crossing a 0.10 line fails until each doc is reviewed and re-stamped. |
...
├── paycalc-transfer.js     ← the pay-data BACKUP FORMAT + import rules (v19.16): `selectBackupKeys` (a prefix scan, not an enumerated list — an enumerated list silently drops a key type added later, the SW-precache failure mode), `summarise`, `buildBackup` (values carried as RAW STRINGS, never re-parsed, so a backup survives a schema change it predates), `validateBackup`, `rekeyEntries`, `backupFilename`. Exists because paycalc data lives in localStorage, which is per-ORIGIN — so it does NOT follow a member from the GitHub Pages mirror to `myb-roster.web.app`, nor to a new phone. Everything else the app holds is in Firestore and moves with the account; this one thing deliberately does not (pay data is kept off the server — ARCHITECTURE_PLAN.md/MILLER_ACTUALS), which is why a Firestore transfer doc was rejected. Every function PURE: the import path decides whether to overwrite someone's entire pay history, so its rules must be testable without a browser. `validateBackup` is the TRUST BOUNDARY (a backup is a file from somewhere — restoring must never be a way to write arbitrary localStorage: paycalc key shape only, never a DEVICE_KEYS flag, string values only, every key under the slug the blob claims). **Identity rule = option A:** another member's backup is REFUSED, because staff share devices — which is exactly why the per-member namespacing exists. Tested by paycalc-transfer.test.mjs
├── paycalc-transfer-card.js ← the DOM half of the "Move Your Pay Data" card on paycalc.html (v19.16; **renamed v19.18/19.19 — the original "Back up your pay data" sat two cards below "Pay Rise Back Pay" and scanned as BACK PAY data, which is established vocabulary here; a card title must not start with "Back up", though "backup" as a NOUN on the buttons and in the filename collides with nothing**): `initTransferCard()` — summary, Download / Copy as text / Restore from a file / Restore from pasted text, and the `#payTransferCard` deep link from Settings (expands the card; a bare fragment jump lands on a collapsed header). Owns no RULE — every decision comes from paycalc-transfer.js. Restore is a **REPLACE, never a merge** (a half-merged pay history is worse than either version): surplus keys are `lsDel`'d, not blanked — a key set to `''` leaves a value the app would later try to parse, and a key the backup lacks must genuinely disappear or the "replace" is a merge wearing a replace's label. **v19.17 — two defects found by driving the real UI, both measured before and after: (1)** the ladder WROTE FIRST, verified by reading back, and only then removed the surplus. The obvious wipe-then-write order is catastrophic here because `lsSet` SWALLOWS a storage error (ls.js, for iOS private mode) — so on a device that had stopped accepting writes the wipe succeeded, the write silently did nothing, and the member was told "Restored" as their pay history vanished; a `try/catch` around `lsSet` is dead code, reading the values back is the only signal. **(2)** every control is disabled when the member cannot be identified — **the paste pair included**, whose omission left one paste able to delete two members' pay history on a shared device (`validateBackup` now fails closed on an empty slug too). Also: `#payTransferCard` joins the print hide-list, and the deep link re-scrolls after layout settles (cancelled by a real gesture, never a scrollY delta — scroll anchoring moves `scrollY` by itself and defeats a delta guard)
...
├── links-fatigue.js        ← the ORR good-practice FATIGUE FACTORS (Dec 2021 guide, **p3** — 24 factors in five families) assessed against a link design (v19.46, LINKS_DEC2026_PLAN.md package 2). Exports `assessFatigue` plus the pure rules it composes (`dutyMinutes`, `isEarlyStart`, `coversNightWindow`, `longestRunBetween48hBreaks`, `earlyBlocksWithShortRecovery`, `maxHoursInAny7Days`, `startTimeJumps`, `rotationDirection`). **It reports factors PRESENT — it never passes or fails a design**, because the ORR is explicit these are not prescriptive limits; a red/green rendering would misrepresent the guidance it quotes. The dominant risk is FALSE ASSURANCE, so every rule reports its status including CLEAR and NOT-APPLICABLE — silence must never be the same shape as compliance, and the night-shift family flips from n/a to present the moment any duty reaches into 00:00–05:00. **FF11 is not the consecutive-worked-days check it resembles**: a single rest day is not a 48h break, so six-on/one-off/six-on is one 12-shift run, and a rotation with no 48h break anywhere returns every worked day rather than the sequence length (the first draft returned the length, counting rest days as shifts — caught by its own test). **`family` is a GROUP NAME and must never state a verdict** (v19.52): it was `'Night (not applicable)'`, and since the panel renders the family as a heading, a design that acquired a night duty printed that all-clear headline over seven rows that had all just turned `present`. Unavoidable factors are marked `standing` not `present` (every 06:20 duty is an FF2 early, so flagging half the rotation every time is how a check gets ignored); FF17/FF18/FF19 carry `confirm: true` because their interpretation is not settled. Hours totals are a FLOOR — SPARE carries no times. **The v19.48 regression pass fixed four rules that ALL failed the same way — flattering the design**, which is the exact risk the module was written to guard against: FF13's status was hardcoded `clear` (a green tick sitting directly beneath the amber short-turnaround row it is the same finding as — it is now read from `runDesignChecks`; **never hardcode a status here**); FF4 called itself "not applicable" when very-early duties existed but none ran over 8h, contradicting the FF3 row above it; `earlyBlocksWithShortRecovery` was the one rule that did not lap the rotation, so a block straddling line 28 → line 1 was cut in half and reported as nothing; and `coversNightWindow` sampled in 15-minute hops, so `23:50-00:05` returned false while `23:00-00:10` returned true. Tested by links-fatigue.test.mjs
├── links-adjacency.js      ← what happens BETWEEN the lines — the ORDER they sit in (v19.58). `OBJECTIVES`, `lineStartProfile`, `weekStep`, `breakLength`, `boundaryRest`, `scoreOrder`, `cost`, `reorderLines`, `applyOrder`. A whole class of a design's quality lives in which line follows which, because you work line w one week and w+1 the next. **Reordering is FREE with respect to coverage** — daily cover is "how many lines work shift X on day D", and permuting rows leaves that multiset identical (asserted by a test; if it fails, the reorder has started moving cells rather than rows) — so the four objectives compete only with EACH OTHER. **Switches, not a formula** (owner): turning one on takes freedom from the rest, so the tool shows what each cost rather than blending them into a score that hides the trade. Two rules that are easy to undo: a SPARE week must not read as a gentle transition (it carries no times, so the step is UNMEASURABLE — scoring that as "no change" would teach the optimiser to park spare weeks between the two harshest lines and report a beautiful number), and days-off is not contracted-days-GIVEN (Sunday is not contracted, so Sat+Sun is a two-day break but one given day; rolled into one figure a design that never rosters Sundays scores as generous while giving nothing away). Deterministic — no randomness, so a designer can re-run and get their design back. Tested by links-adjacency.test.mjs
├── links-demand.js         ← the SERVICE a design has to cover — trains per hour at Marylebone (v19.56, LINKS_DEC2026_PLAN package 3). `DEC_2026_MOVEMENTS` (every passenger movement as `[minute, cars, isArrival]`; a recovered length is CAPPED at 9, the route maximum — the two-diagram sum rule produced three impossible values and the first plausibility test missed all three because it checked each hour's MEAN), the derived `DEC_2026_DEMAND` hourly curve, `DEC_2026_SOURCE`, `demandAt`, `peakCars`, `peakHours`, `demandBucket`, `movementsOutside`, `summariseDemand`, `describeHours`/`describeMovements`. **Demand = arrivals AS WELL AS departures, weighted by train length** (owner, Aug 2026 — the answer to the plan's open question 1). Both figures are kept rather than blended because they DISAGREE: by movement count the weekday morning and evening tie at 23, but by cars the evening is ~10% heavier and the peak hour moves 08:00 → 17:00. **Stored as TIMES, not as an hourly table, and that is load-bearing** — the first version held hourly buckets, and since the Sunday window closes at 23:25 the 23:00 hour counts as "staffed", so the five December 2026 movements after it — the one live finding the whole feature exists to surface — were invisible. A unit test passed too, because it used a synthetic whole-hour Sunday window instead of the real one. The hourly curve is now DERIVED from the times, so the shading and the boundary check cannot describe different timetables. `summariseDemand` splits a FINDING (`uncovered` — station open, trains running, nobody on duty; per DAY, at hour resolution) from a FACT (`outside` — trains when the station is shut; per day CLASS, to the MINUTE, split before/after). That split is the anti-cry-wolf rule: the last trains and the 05:5x first departure are outside the window on every design there has ever been, so scoring them would flag every design forever and teach the reader to skip the row. It never scores a design — no "covers 87% of demand" figure, same principle as links-fatigue.js. Tested by links-demand.test.mjs
├── links-window.js         ← the OPERATING WINDOW of a design — when the station is staffed (v19.54, LINKS_DEC2026_PLAN package 1). `DEFAULT_WINDOW` (Mon–Sat 06:20–23:55 · Sun 07:15–23:25), `normaliseWindow`, `isHourStaffed`, `heatSpan`, `formatWindow`, `windowsDiffer`, `isDefaultWindow`, `canonicaliseWindowTime`. **The reason is stronger than the plan recorded:** the heat map derived its own span from the design — first worked hour to last — and flagged a gap only strictly BETWEEN them, so missing cover at either END of the day was invisible; the span just shrank to fit and those hours left the table. A 28-line design where everybody finishes at 14:20, leaving the station unstaffed until the 23:55 close, rendered as solid colour with **zero gaps flagged**. It now shows **71**. Stored PER DESIGN so a proposal can test a moved boundary (the live case is Sunday, where five Dec 2026 movements fall after the 23:25 finish). **The trap that comes with per-design: compare mode diffs CELLS, not windows** — so both compare columns and the printed sheet MUST state it, or the override becomes a way to make an unfair comparison look fair. `normaliseWindow` falls back PER ROW but never per FIELD: pairing a stored start with a default end would invent a window nobody chose and then print it as theirs. The editor uses TEXT inputs, not `<input type="time">` — Chromium renders that widget in the OS's 12-hour format **even at en-GB** (measured), which would put "11:55 PM" beside a grid reading 23:55. Tested by links-window.test.mjs
├── links-deletion.js       ← the PURE rules behind "Recently deleted" in the Links workspace (v19.41): `isDeleted`, `isPurgeable`, `purgeableIds`, `daysLeft`, `deletedLabel`, `sortByDeleted`, `canSoftDelete`, `SOFT_DELETE_RETENTION_DAYS` (30). Delete was permanent — any designer could destroy any design behind one confirm, with no archive and no ownership check; accepted at two designers, and the agreed trigger to revisit (a third) fired at v19.40. A design now carries `deletedAt`/`deletedBy`, is hidden from the picker, and is restorable for 30 days. **`isDeleted` and `isPurgeable` are deliberately NOT mirrors.** They disagree about one state — an UNRESOLVED `deletedAt`, which is what a `serverTimestamp()` write reads back as on the writing device until the server resolves it. That must count as DELETED (or the design sits in the picker on the very device that just binned it, until a round trip it may not get offline) and must NOT count as PURGEABLE (an age you cannot read is not an age that has expired). The asymmetry is the whole design: hiding a design wrongly is a nuisance, destroying one is the failure the feature exists to prevent — so `isPurgeable` also refuses a FUTURE `deletedAt`, because the purge runs on the client like every other prune here and is only as trustworthy as `Date.now()`. Tested by links-deletion.test.mjs
├── links-design.js         ← pure link-design maths: ROTATING_LINES (the ONE declaration of 28 — v19.38; it was a literal in three files kept in step by a comment), canonicaliseShift/normalisePatterns (v19.38 — pad a legacy unpadded time on load so the module has ONE time format; `classifyShift` now reads the hour through the same strict parser the coverage maths uses, so the two can no longer disagree about what a time IS: "6:00-14:00" used to classify as a normal early while startMinutes returned null, counting in the day totals yet ABSENT from the hourly heat map and exempt from every turnaround check), classifyShift, normaliseCustomShift, calcCoverage, calcHourlyCoverage, generatePatterns (v19.58 — SPARE is a whole WEEK, `spareLines` reserving whole lines spread around the wheel, not a per-day headcount fed to the sliding window: that scattered a person's spare days across the week while the daily SP total stayed correct, which is exactly why nobody noticed. The real roster has whole spare weeks only — main lines 1/7/12/17), runDesignChecks, dayClass, `endMinutesAbs` (v19.47 — the ONE reading of a duty that runs past midnight; both former inline copies erred towards *safer than the truth*: the heat map dropped the post-midnight hours entirely and the turnaround check credited a 00:30 finish with ~26h of rest before an 06:20 start instead of 5h50. A wrapping duty now counts on BOTH days, Sat spilling to Sun. Unreachable from the CEA link — duties finish 23:55 and `normaliseCustomShift` refuses a wrapping value; keep that ban)
├── links-analysis.js       ← links.html read-only analysis panels — Coverage heat map + Design quality checks (extracted from links-app.js v17.70). `initLinksAnalysis({ getDesign, getBaseline }) → { renderCoverageChart, renderDesignChecks, renderSummary }` (the third being the live strip in the grid card's sticky save bar, v19.57 — the analysis sits ~1,600px below the fold, so an edit's effect was invisible without scrolling two screens away and back); pure render-from-a-pure-result (maths in links-design.js + links-fatigue.js), reads only the live active design via the getter, no coordinator save/dirty state. **Every class it emits must exist in links.css** — a static contract added v19.48 after v19.46 shipped ~15 rows on `check-info`, a class no stylesheet defines: nothing threw, every behavioural test passed (they assert text), and those rows rendered with NO surface beside neighbours that had one. The guard matches with `(?![\w-])` not `\b`, because a hyphen is a word boundary and `\.check-info\b` matches inside `.check-info-icon` — the first version passed against the very bug it was written for. **It was generic within `check-*` only, which is the same silent gap one level up** (v19.56): the demand rows arrived under `dem-*`/`cov-demand` and not one of them was policed. It now covers a PREFIX SET, plus a second test asserting the module emits no family outside it — so a new family is a loud failure rather than an invisible hole. Tested by links-analysis.test.mjs
...
├── LINKS_DEC2026_PLAN.md    ← the Links workspace vs the **December 2026 timetable change** ("Dec 26" is the TT change, NOT Boxing Day — this repo separately forces 25/26 Dec to rest days, so the shorthand collides). Records the measured Dec 2026 service at Marylebone (306 Mon–Fri / 215 Sat / 188 Sun passenger movements, with the hourly shape), the CEA operating window (Mon–Sat 06:20–23:55, Sun 07:15–23:25) and the ONE boundary the new timetable breaks — five Sunday movements fall after the 23:25 finish, including a 23:45 departure. Then the tool gap: ORR p3 lists 24 fatigue factors and `runDesignChecks` covers two of them. Five work packages (reviewed and re-ordered — the fatigue checks depend on nothing and go first; the per-design operating-window setting unblocks the overlay and the generator targets). Carries the measured BASELINE of the live link (main cycle 15 consecutive worked days, bilingual 14 — both already past FF11's 13 — and zero short turnarounds on either), because a proposal's numbers are unreadable without it. Not version-stamped; not a runtime asset.
...
├── paycalc-transfer.test.mjs ← tests for paycalc-transfer.js — the pay-data backup/restore. The two things worth testing hardest are the TRUST BOUNDARY (a backup is a file the member obtained from somewhere; a foreign key, a smuggled DEVICE key, a punctuation/traversal-looking key, a non-string value and a key outside the claimed slug are each refused) and the ROUND TRIP (every value survives byte-identically — these are pay figures, not approximations). Also covers the option-A identity rule and legacy unnamespaced data, whose ambiguous tail (`myb_pc_ytd_pay_2026_27` — is the slug `ytd`?) is why the SOURCE slug is recorded rather than guessed. Teeth-verified four ways: drop the import key check, allow a different member, stop excluding DEVICE_KEYS from the export, and guess the slug instead of reading the recorded one. No mocks; part of test:hygiene
├── paycalc-key-parity.test.mjs ← the answer to "does the pay-data transfer need checking every time the pay calculator changes?" (owner, Jul 2026) — yes, and a habit someone has to remember is the weakest form of that, so it is a static guard instead (v19.28). `paycalc-transfer.js` carries whatever localStorage holds, so it needs no edit when a pay FEATURE changes; what breaks it is a change to key SHAPE, and both ways that happens are SILENT. A key built without `pcPrefix()` is not namespaced (two members on a shared device read each other's pay — the thing v14.11 exists to prevent) AND is invisible to `selectBackupKeys`, so it is simply absent from every backup. A key whose tail is not `[A-Za-z0-9_]+` fails the transfer's `KEY_RE` and is dropped from every backup while the member is told "Restored". Nothing throws, nothing logs. Three contracts + the rule pinned: every hardcoded `myb_pc_*` literal is in `DEVICE_KEYS` (a literal is BY DEFINITION un-namespaced, so it is either a declared device flag or a privacy bug); `DEVICE_KEYS` has no ghosts; every `pcPrefix()`-built tail is KEY_RE-safe; and every `TAX_YEARS` label survives the `/`→`_` transform — that table is hand-edited every April, and a label like `2027/28 (est)` would quietly break `hpp_est_`/`bp_state_`/`ytd_pay_` for that whole year. `paycalc-transfer.test.mjs` cannot catch any of this: it tests the rules against keys it invents itself, so it can never notice a REAL key that stopped conforming. Note the tail-extraction regex captures to the end of the literal segment, **not** with a `[A-Za-z0-9_]*` class — a class listing only the legal characters can never see an illegal one (given `${p}sl-paid-off` it captures `sl` and passes; it did, first time round). Part of test:hygiene. Teeth-verified four ways, each failing its own contract
├── roster-prompt-parity.test.mjs ← the roster-parse PROMPT is code, and nothing was checking it (v19.31). Written after a staff report — "the roster uploader only gets AL right 50% of the time" — which was neither OCR nor the parser. In the real PDF a cell's SECOND line holds the train duty code on a worked day ("CEA 24") and the STATUS code on a non-worked one ("AL"); the prompt said "Duty/diagram codes on a second line … ignore them. Only the first line of each cell is the shift value" AND "Blank … = RD", which compose into: ignore the AL, see an empty first line, return RD. The model was being instructed to discard annual leave. **The 50% was the signature of an INSTRUCTION CONFLICT** — a contradiction resolved differently run to run — not of a hard-to-read document, which is why it looked like flaky vision. It survived because **RD sits on that same second line and is right BY ACCIDENT** ("ignore the second line" + "blank = RD" = RD, which is what an RD cell means anyway), so the rule only did damage where the correct answer DIFFERS from a rest day — AL, SPARE, and every absence code — reading most of the table perfectly while silently dropping leave and sickness. Two contracts, deliberately different: (1) **the regression** — no line-POSITION rule that can swallow a status code, and the layout rule must say a second-line status code IS the value (this is the one that would have caught it); (2) **code-table parity** — every code `normaliseShift` accepts is documented in the prompt, which would NOT have caught it (AL was documented) but catches the mirror-image failure, where teaching the parser a code the prompt never asks for leaves that branch unreachable because the AI never emits it. Prose, so read as source text (the prompt is a template literal inside a request handler and can't be imported); extraction THROWS if the anchors stop matching rather than vacuously passing. Part of test:hygiene. Teeth-verified four ways
...
│   ├── paycalc.spec.js     ← paycalc in-place login, signed-in period selector, desktop workspace geometry (1024–1440px + short height), one-time-notice stacking, and the **backup/restore** wiring (v19.16: the Settings deep link lands on the card OPEN, a paste→confirm→reload round trip puts the exact figure back, another member's backup is refused and writes nothing). The rules are unit-tested in `paycalc-transfer.test.mjs`; what only a real browser can prove is the WIRING — that the paste box reaches the ladder and the confirmation actually gates a destructive write. **Note the round-trip test writes its payload key AFTER load, never in an `addInitScript`** — init scripts re-run on the post-restore reload, so a seeded value reappears by itself and the test passes with the restore's write deleted (it did, first time round)
│   ├── pages.spec.js       ← settings/operations/links login + signed-in render, operations desktop columns + long-email layout + App-speed card, admin touch-layout blowout guard, FIP jump-link + malformed-hash, the Settings **Pay Calculator Data** pointer card (v19.16 — a pointer to the paycalc backup card, deliberately not a second copy of the controls), and the **roster review picker** (v19.35 — a flagged cell resolved from the review table: the picker only appears where the server sent two readings, a pick reaches the actual WRITE and not merely the "Save N changes" counter — those are two separate passes and asserting the counter alone had no teeth — Skip reverts to writing nothing, and Skip never wears the success colour, which the visual baseline provably cannot see)
...
│   ├── visual.spec.js      ← visual-regression baselines (Section B / F-VIS, v17.73): runs ONLY via `npm run test:visual` under `playwright.visual.mjs`. Clock-pinned + Firebase-stubbed + fixed-member → deterministic fixed-viewport screenshots vs committed baselines in `e2e/visual-baselines/` (18: the app surfaces incl. the accepted desktop voids, the 4 static guides, three OVERLAYS added v19.03 — About, App Notices, the signed-out login card — and the Operations **Usage card populated**, v19.25, and the Operations **roster review table**, v19.34, and the Links **Recently deleted** panel, v19.43). That last one is the clearest case for the whole suite: the panel shipped at v19.41 with the bare `.lb-content` — which is only the transform/scroll base, not a surface — so it rendered as a TRANSPARENT box with the heading and prose in navy directly on the dimmed backdrop, and all 256 e2e tests passed because they assert text and behaviour. Overlays had no pixel coverage at all until v19.03, which is a large share of the app's surface area and exactly where width/padding drift lives. The Usage card was the same gap for a different reason: operations is baselined with its cards COLLAPSED, so the app's most composed surface — four sections, three bar groups, a stacked bar, a legend — had none, and three v19.24 defects in it were caught only by hand-rendering. It needs DATA to be worth capturing, so the test rewrites `firebase-client.js` to return fixed stats; that injection **THROWS if its anchor stops matching**, and the test asserts the fixture reached the DOM before capturing — a silent no-op would render the EMPTY state, and the next baseline regeneration would lock in a green test that had quietly stopped testing anything.) Catches composition drift from a CSS/token/layout change — but only WHEN RUN: it is **opt-in and deliberately NOT a CI gate** (pixel diffs are environment-sensitive — the baselines were generated in this dev-container's headless Chromium, so a CI runner on a different image would flake). So a CSS/layout change can still ship un-screenshotted; run `npm run test:visual` (and re-baseline intended changes) before merging one. EXCLUDED from the smoke run. **Mobile calendar @390px deliberately NOT baselined** (its fractional 7-col grid flakes pixel diffing). Regenerate: `npm run test:visual -- --update-snapshots=all` (`=all` is load-bearing — a bare `--update-snapshots` only rewrites baselines whose comparison FAILED, so a baseline that drifted inside the tolerance could never be refreshed)
...
**Run all tests:**
```
npm test              # test:hygiene + test:parse + test:unit (~1990 tests across 78 root test files)
...
deletedAt   Optional (v19.41) — Firestore server timestamp. PRESENT = in the "Recently deleted"
            bin (hidden from the picker, restorable for 30 days); ABSENT = live. Restore clears
