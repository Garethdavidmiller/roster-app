...
├── railcard-guide.js       ← JS for railcard-guide.html: print, chip-bar navigation. The PAGE was rebuilt for the Marylebone gateline at v20.38: it separates the RAILCARD's discount rule from the TICKET's validity from CHILTERN's revenue procedure, groups the cards by the KIND of morning rule (none / minimum fare / fixed cutoff / follows-the-ticket), and carries a directional Chiltern Off-Peak panel in place of the invented `~10:00` shortcuts. **Two errors it shipped for months are pinned by teeth-verified tests** — Network truncated at Banbury, and "16-17 Saver is digital only". Full design + the source audit: `.claude/rules/guide-pages.md`
