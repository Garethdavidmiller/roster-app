...
| Current app version | `6.37` (check `roster-data.js` — `APP_VERSION` is the authoritative source) |
...
| Two separate type pill lists in admin | Per-row pills are generated by `renderWeekGrid()` in `admin-app.js` (JS template string, line ~782). Bulk bar pills are in `admin.html` (static HTML, line ~2179). **Both lists must be kept in sync.** Adding a new type requires updating both. The order must also match. |
...
| `'ALLOCATED'` | ✅ Allocated | Spare week — actual shift has been assigned. Stored as a time string (HH:MM-HH:MM) with override type `allocated`. |
...
type         string     "spare_shift" | "allocated" | "overtime" | "rdw" | "swap" |
