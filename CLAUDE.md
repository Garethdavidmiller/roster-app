...
| Sticky take-home bar (`#stickyTotal`) | Fixed bottom bar on ALL widths (v16.12 — desktop version capped to `--content-max-width`, centred; it replaced `.result-card`'s `position: sticky`, which painted over and click-blocked the right column when pinned — never re-add it). `IntersectionObserver` + `body.sticky-active`. See `.claude/rules/paycalc.md`. |
