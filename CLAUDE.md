...
| Current app version | `21.30` (latest 0.10 milestone; exact value in `roster-data.js` — `APP_VERSION` is authoritative). The version stamp in **every** doc (this file, AI_MAP, OPERATIONS_REFERENCE, KNOWN_LIMITATIONS, ROADMAP) is enforced against the latest 0.10 milestone by `sw-asset-check.test.mjs` and `githooks/pre-commit` — a bump crossing a 0.10 line fails until each doc is reviewed and re-stamped. |
...
├── admin-tasks.js ← the four jobs Admin carries, and the chip row that focuses one: `ADMIN_TASKS`, `DEFAULT_TASK`, `taskForHash`, `initialTask`, `initAdminTasks`. A FOCUS control, not navigation — the selected member is the context all four share, so four pages would mean reselecting somebody to do the second thing. **It reorders nothing** (the two-column grid's row order is load-bearing for the mobile stack) and **it focuses a card by clicking that card's own collapse control**, never by setting classes — the state is three things kept in step, and a second writer of one of them is how an arrow ends up pointing the wrong way. The default is Change a Shift for everyone, every time. Tested by admin-tasks.test.mjs + e2e
...
├── admin-tasks.test.mjs ← the pure half of the task row, pinned on the two things that make the page PREDICTABLE: the default never varies (not by role, not by last visit — a page that opens somewhere different each time defeats the muscle memory that is the point), and a deep link and the chip row never disagree. The DOM half is a browser test, because the mechanism is a click on the shared collapse control. Part of test:hygiene
