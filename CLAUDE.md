...
| Current app version | `16.70` (latest 0.10 milestone; exact value in `roster-data.js` — `APP_VERSION` is authoritative). The version stamp in **every** doc (this file, AI_MAP, OPERATIONS_REFERENCE, KNOWN_LIMITATIONS, ROADMAP) is enforced against the latest 0.10 milestone by `sw-asset-check.test.mjs` and `githooks/pre-commit` — a bump crossing a 0.10 line fails until each doc is reviewed and re-stamped. |
...
│   └── barlow-semicondensed-latin-700.woff2 ← self-hosted display face (v16.73) — HERO moments ONLY (paycalc take-home £ + sticky bar £, calendar month heading) via `--font-display`; SIL OFL, vendored from `@fontsource/barlow-semi-condensed` (`npm pack`, extract the latin-700 woff2). Do not add weights or apply to body text.
...
| **Display face — hero moments only (v16.73)** | `fonts/barlow-semicondensed-latin-700.woff2` (Barlow Semi Condensed Bold, SIL OFL, self-hosted + SW-precached like Inter) is the app's display voice — a signage-rooted grotesque with a rail departure-board flavour. Used via `--font-display` in exactly THREE places: paycalc `.net-amount`, the `#stickyTotal` amount, and the calendar `.month-year`. **Restraint is the rule:** do not add weights, do not apply it to body text, buttons, or card headers — spreading it destroys the hero effect. Falls back to bold Inter (`--font-display` ends in `var(--font-sans)`). |
