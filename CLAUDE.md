...
├── paycalc-transfer-card.js ← the DOM half of the "Back up your pay data" card on paycalc.html (v19.16): `initTransferCard()` — summary, Download / Copy as text / Restore from a file / Restore from pasted text, and the `#payTransferCard` deep link from Settings (expands the card; a bare fragment jump lands on a collapsed header). Owns no RULE — every decision comes from paycalc-transfer.js. Restore is a **REPLACE, never a merge** (a half-merged pay history is worse than either version): existing keys are `lsDel`'d, not blanked — a key set to `''` leaves a value the app would later try to parse, and a key the backup lacks must genuinely disappear or the "replace" is a merge wearing a replace's label
