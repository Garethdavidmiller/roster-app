...
| Documentation milestone | `23.90` — the 0.10 checkpoint these docs are written against. **This row is deliberately NOT the current version**, and used to be: it read `23.23` while the app shipped 23.24, then 23.25, then 23.26 — a figure that rots on every release and that nobody bumps, because bumping it is not part of shipping. The exact version lives in `roster-data.js`, where `APP_VERSION` is authoritative and is the only place a reader should look for it. What the guard actually requires here is the MILESTONE, so that is what this row now states. The version stamp in **every** doc (this file, AI_MAP, OPERATIONS_REFERENCE, KNOWN_LIMITATIONS, ROADMAP) is enforced against the latest 0.10 milestone by `sw-asset-check.test.mjs` and `githooks/pre-commit` — a bump crossing a 0.10 line fails until each doc is reviewed and re-stamped. |
...
**Example payslips for testing:** the pay-maths regression net is thirteen REAL payslips, and since v23.71 they are **not in the repository** — `test-fixtures/payslip-actuals.local.js` is gitignored (`*.local.js`), because `firebase.json`'s `test-fixtures/**` exclusion governs Firebase Hosting and the GitHub Pages mirror serves the repo root obeying no ignore list (measured: 200 on the mirror, 404 on the canonical origin). Keeping the bytes out is the only rule both origins obey; `sw-asset-check.test.mjs` guards the `.gitignore` line itself. **On any checkout but the owner's the payslip regression SKIPS, loudly and by name** — `paycalc.test.mjs` says which assertions did not run, because a loop over an absent fixture would otherwise register nothing and report a smaller green number that reads exactly like a passing one. `test-fixtures/payslip-actuals.example.js` documents the shape and is never imported: a green run against invented figures would report the regression as PASSING while proving only that the calculator agrees with itself. History was deliberately not rewritten — see KNOWN_LIMITATIONS.md. A **device-local** actuals overlay is unchanged and developer-only: `paycalc-app.js` reads `readPayslipActuals()` from localStorage, gated to `G. Miller`, imported per device and never served. Full reasoning: AUTH_ARCHITECTURE.md → MILLER_ACTUALS. **ABSENT is a skip; BROKEN is a failure** (external review) — the v23.71 cut caught EVERY import error as "not on this checkout", so a fixture that was present but malformed downgraded the repo's most valuable verification to a silent skip on the one machine holding the data. Presence is now read off the FILESYSTEM rather than inferred from an error code (`ERR_MODULE_NOT_FOUND` is also what a missing import INSIDE the fixture raises), the import runs with no catch, and a fixture that imports but exports nothing usable fails too rather than reading as absent. When changing pay maths (tax, NI, thresholds, variable pay), run **`npm run test:payslip-local`** — it requires the fixture and FAILS if it is missing, so a pay-maths release has one command whose green means the real comparison actually ran, which an ordinary honest skip cannot say; use `paycalc-hpp.test.mjs` for `_varPayForPeriod` regressions. **But be precise about what that green MEANS, because this sentence used to overstate it:** all thirteen payslips are **2025/26**, a year that has ENDED. Measured 16 Sep 2026 — a £50/yr change to the **2026/27** personal allowance, inside every estimate a member sees today, left the WHOLE estate green. The current year is now locked by **`paycalc-current-year.test.mjs`**, which is built on the FREEZE rather than on a tautology: 2025/26 is payslip-verified, the Westminster freeze runs to Apr 2031, so the equalities it asserts carry real-payslip evidence into the current year. The two genuinely uprated sets (Scottish Starter/Basic, the five SL3 thresholds) are literals with their source named — a change-detector, not a truth-oracle, and its header says so.
