...
| Current app version | `21.90` (latest 0.10 milestone; exact value in `roster-data.js` — `APP_VERSION` is authoritative). The version stamp in **every** doc (this file, AI_MAP, OPERATIONS_REFERENCE, KNOWN_LIMITATIONS, ROADMAP) is enforced against the latest 0.10 milestone by `sw-asset-check.test.mjs` and `githooks/pre-commit` — a bump crossing a 0.10 line fails until each doc is reviewed and re-stamped. |
...
├── al-entitlement.js ← which ANNUAL-LEAVE days consume entitlement, and where that leaves a member: `consumesEntitlement`, `countedAlDates`, `alPosition`. **A SWAP is not overtime** (v21.55): booking AL by one rule (`isWorkingDate`, override-aware) and charging for it by another (the base roster) gave a swapped-in day free leave, so `consumesEntitlement` now reads the override too — via `isContractedWorkOverride`, in which `shift` counts and `rdw` deliberately does not. The set feeding `projectAnnualLeaveOverage` was built by hand at FOUR sites in three different ways — the AL banner, the week-grid cap check, `admin-al.js` and the calendar's own AL lightbox — so a member's balance could disagree with the figure the save path acted on. **A Sunday never counts; the override underneath (or the `replacedType` an AL doc preserved) answers next; only then does the base roster decide, where a rest day never counts** — that base fallback also settles leave dated before a member joined, which is why no separate start-date rule belongs here; each site had a different subset. Pure; tested by al-entitlement.test.mjs
...
├── operations-speed.js     ← the App Speed card: `initPageSpeedCard` — the two journeys, the per-page table, and the three "Why some are slower" blocks (boot stages, the start ladder, the dimensional splits)
...
├── admin-week-editor.js ← the seven-day grid, its pills, the bulk bar, and everything STAGED but not saved: `renderWeekGrid`, `buildWeekGridInto`, `updateWeekNavLabel`, `updateSaveBtn`, `resetBulkPills`, `resetStagedRows`, `_hasStagedEdits`. **Staged counts BOTH directions** — a row given a type and a prefilled row un-ticked — because counting only the first discarded staged removals on a background refresh while the dirty flag stayed set. **An unread member draws a loading state, and a failed load a distinct failed one**: painting seven base-roster days is indistinguishable from a clear week
├── admin-saved-changes.js ← the Saved Changes list — what is on record, and un-recording it: `renderTable`, `resetTableMemberFilter`, the month/member filters and both delete paths. **It may not claim completeness it does not have**: `coversAllStaff()` is asked at RENDER time, not when the All-staff toggle is pressed, because the toggle flips a flag and then starts a load that can fail — and a list of three labelled "(all staff)" is a manager answering "has anyone else booked that week?" with no
...
├── admin-week-swipe.js ← the week-grid SWIPE on admin.html: `initAdminWeekSwipe`. A pointer state machine with a carousel attached — it decides when a drag is a swipe rather than a tap or a scroll, builds the adjacent panels, animates the commit and holds a cooldown so one gesture cannot advance two weeks. It knows nothing about rosters, overrides, leave or saving: what a week change MEANS to Admin arrives as `onWeekCommitted` and `onSettled`, and whether it may happen at all as `canNavigate`. **Deliberately NOT shared with calendar-swipe.js** — same-looking, different thing (a month not a week, no unsaved-work guard, no adjacent build), and merging them would buy one handler behind a mode flag. Two standing rules it must keep: Pointer Events never Touch Events, and `setPointerCapture` on the GRID not the clip. Covered by e2e/pages.spec.js, written BEFORE the extraction because the gesture had none
...
├── paycalc-sticky-total.js ← the bar that keeps the take-home figure on screen: `initPaycalcStickyTotal`. Three inputs (is the result in view · is a field focused · where is the visual viewport) and one output (shown, and how far translated). It touches no money, no period and no storage — `calculate()` writes the figure and does not know this file exists. Every guard in it is the fix to a reported iOS behaviour and each is conservative in the same direction: no pagehide-disconnect (an observer survives bfcache; disconnecting froze the bar over the footer), the bar is PINNED above the keyboard rather than hidden behind it (hiding it WAS the "pay total is slow to update on iOS" report — the figure was invisible, not late), and the translate composes with the desktop centring rule. None of it is reproducible headless
...
├── paycalc-pension.js ← WHEN a member was in the pension scheme, not merely whether: `parsePensionTimeline` (a trust boundary), `isOptedOutAt`, `withPensionChange` and the two edits the card actually makes — `withOptOutStartAt` (move a spell's start) and `withRejoinAt` (end one) — plus `optOutStartsAt`, `migrateLegacyOptOut`. The v21.64 opt-out was a single boolean, and that shape was the money bug: a period whose pension equals the default stores `null` and re-reads it, so making the default £0 rewrote every historical payslip — measured at £160.78 → £0.00 and £115.92 onto one 2025/26 take-home. **An empty timeline means CONTRIBUTING, a payslip before the first change is never touched by it, and a timeline that will not parse whole is refused whole** — a half-read one is a valid timeline that says something else. A timeline rather than one date because auto-enrolment re-enrols opted-out staff about every three years, so a rejoin is expected rather than hypothetical. Pure; tested by paycalc-pension.test.mjs
...
├── upload-commit.js ← what an AMBIGUOUS Firestore commit means after a document upload: `resolveUploadCommit`. A `deadline-exceeded` can be raised AFTER the server committed, and the old retry re-issued the write blind — idempotent against itself and against nobody else, so a competing upload's file reference could be overwritten with a Storage path that upload had already deleted. The live document then 404s and nothing errors. **Resolve by READING**; the three answers (ours / unchanged / somebody else's) are distinguishable, and an unreadable state retries rather than abandoning a file nothing points at. Pure; tested by upload-commit.test.mjs
├── documents-client.js ← the three date-keyed document collections (Huddle · Circular · Newsletter) and the UPLOAD SEQUENCE behind them: `buildDocumentClient`, `assertFileSignature`. **One invariant — a live document must never point at a file that is not there** — and four rules that serve it: check the signature before anything is written, upload to a VERSIONED path so the old file outlives the commit, roll the new object back only on a DEFINITE non-commit, and resolve an ambiguous one by READING. Split from firebase-client.js at v21.90, where it sat between the auth bootstrap, the push writers, the password timestamps, the analytics counters and the error log. Every Firebase handle is INJECTED — that is what keeps the graph acyclic AND what lets the ordering be replayed in Node. Tested by documents-client.test.mjs
...
├── overtime-review-controller.js ← the REVIEWER's side of overtime.html: the planning horizon, week selection, the detail workspace, the creation preview and confirm bar — and the EIGHT pieces of state that have to agree about which week is on screen (`createReviewController`). The page holding both surfaces is deliberate; one coordinator holding both was not. Nothing here is reachable from a member's world and none of this state means anything to them. **Every field exists to stop several asynchronous reads disagreeing** — the two generation tickets, the `selectedWeek` re-check after every await, `horizonByWeek` (where the detail read gets its milestones), and the grade/day lenses, which differ from each other on purpose: grade survives a week switch, day does not, because its values are one window's dates. The four shared DOM helpers are INJECTED rather than duplicated — they are the page's, not the reviewer's
...
├── links-design-store.js ← the design collection's PERSISTENCE LIFECYCLE, and the concurrency protocol that goes with it: `createDesignStore` (load · create · save · rename · softDelete · restore · purge · purgeIfExpired) + `isOfflineFailure`. It exists because that protocol lived in EIGHT functions and twenty-seven Firestore calls in the coordinator, each with its own variant — and an external audit found two silent overwrites in two of them, both the same shape, where fixing each in place did nothing to stop a third being written. **Four rules it makes unwritable**: a read a write depends on happens INSIDE that write's transaction; there is no weaker fallback while online (offline is a different kind of thing, not a lesser degree); the local baseline only advances to a revision this module verified; and **a forced save overrides a competing SAVE, never a DELETE** — two different consents, and only the first is ever given. It holds NO DOM, no dialogs and no decision about what the user sees — a conflict comes back as a result and the workspace decides — which is what lets every Firebase handle be injected and the interleavings replayed. Tested by links-design-store.test.mjs
...
├── links-design.js ← pure link-design maths: `ROTATING_LINES` (the ONE declaration of the rotation length — never write the number down), shift classification/normalisation, coverage, the auto-generator (`generateLink` + waves), `runDesignChecks`, `weeklyHours`, `repairShortRest`/`restBetween` (the MINIMUM-REST gate — the generator repairs a short turnaround by swapping two lines' duties within one day column, which changes no target, and REFUSES what it cannot repair), `lineTotals` (the grid's per-line columns and the days-worked average — **its divisor is the working lines, not the rotation**, unlike the hours average beside it), the worked-run helpers and `endMinutesAbs`. **The invariants an edit can silently break are in the module header** — read it before touching the generator or the hours maths. Tested by links-design.test.mjs
...
├── links-target-sets.js ← SAVED SETS of generator targets: `targetSetFromDoc` (a trust boundary — a corrupt document is refused whole, never half-loaded as a lighter week), `targetSetPayload`, `canOverwriteTargetSet` (the CLIENT copy of the ownership rule — it drives the Save button; the rule the app RELIES on is the same statement in firestore.rules: anyone creates, only the creator or admin overwrites or deletes, `createdBy` pinned to the writer's own name claim on create and immutable on update), `describeSetState` (what the row SAYS and OFFERS — whose the set is and where the table stands relative to it, two questions whose collapse into one is every defect this feature has had) and `sortTargetSets`. Tested by links-target-sets.test.mjs; the server side case-for-case in firestore.rules.test.mjs
...
├── links-default-targets.js ← the table the generator STARTS from: `buildDefaultTargets`, `DEFAULT_COVER_WEEKS`, `OPENING_TURNS`, `CLOSING_TURNS`, `SATURDAY_CLOSING_TURNS`, `SATURDAY_TURNS`, `SUNDAY_TURNS`, `TARGET_DAYS_PER_WEEK`, `EVENING_TURNS_FROM_22`, `DEFAULT_SHIFT_TIMES`, plus `sameTargetTable`/`isSupersededMemory` — a remembered table the app stored ON ITS OWN may not outlive a changed default; one anybody edited is never discarded. `links-seed.js` MEASURES today's roster; this one DESIGNS against the December 2026 service, and it is the default because the seed can no longer pay a 24-line rotation and so met a designer with a refusal. The contract, the cover-week count and the mean duty are one equation, not three preferences — the module header has it written out. Tested by links-default-targets.test.mjs
├── links-import.js ← turning somebody ELSE's roster proposal into a design: `parseCell`, `parseGrid`, `parseDesignImport`, `summariseImport`. Proposals arrive as a photograph, a Word table or a spreadsheet column, and the only route in was 168 taps — enough friction that a colleague's idea never got compared against anything. **It is a TRUST BOUNDARY, modelled on `validateBackup`**: pasted text is a claim about a design, and it becomes either a design or a refusal, **never a half-design**. An unreadable cell is refused BY NAME, never defaulted to a rest day — an import that "succeeds" four duties light reads as a lighter week, and every panel downstream then reports confidently about it. It computes no hours and passes no verdict; the workspace's own panels assess it. Tested by links-import.test.mjs
...
├── LATENCY_PLAN.md ← sign-in and Calendar START latency: what shipped, and the DECISION RULE for what has not. Phases 2 and 3 are deliberately unstarted — the start ladder that chooses between them has only just begun collecting, and picking one before it reports is the guess the measurement exists to stop. Holds the sequence and the trigger only; what is measured lives in `perf-reporter.js`/`perf-stats.js`, and the invariants any change must not break in `CALENDAR_DATA.md`. Not version-stamped; not a runtime asset.
...
├── admin-override-store.test.mjs ← the store EXECUTED, because its defects are not calculations — they are the order several asynchronous reads write the cache in, and which of them was allowed to run at all. A perfect `hasAuthorityFor` still answers about a member whose read was silently skipped. Organised by what a wrong answer COSTS: the load never happens (a week grid on "Loading…" for ever and a Saved Changes card stating a member has none, which is what a manager then acts on) · the cache claims to know what it never read · it keeps what has been deleted · the screen says something untrue. `getDocs` is driven by a handler that can HOLD a read open, and each test imports the store under its own query string, because the cache is module state with no public reset. (--experimental-test-module-mocks). Teeth-verified by ten mutations
...
├── status-glyph-parity.test.mjs ← the same rule, enforced against the whole tree: no live region in served markup leads with a bare glyph, no module assigns a glyph-leading string straight to `.textContent`, no `innerHTML` template leads with one, and nothing calls `setStatus` without importing it. It found the two static `role="alert"` warnings in paycalc.html that predate the a11y gate, and two local functions ALSO called `setStatus` shadowing the shared one. The button exemption is by NAME and therefore loose — stated in the header rather than left to be noticed. Part of test:hygiene. Teeth-verified
├── upload-commit.test.mjs ← the ambiguous-commit verdict, organised by what each wrong answer COSTS — `retry` when somebody superseded us overwrites their file reference (the shipped defect), `committed` when nothing committed leaves a file nothing points at, `superseded` when nothing did is a refusal the admin can simply retry. Part of test:hygiene. Teeth-verified by four mutations
├── documents-client.test.mjs ← the upload sequence EXECUTED against a fake Firestore and a fake Storage, which is only possible because the handles are injected. Organised by what each wrong answer COSTS, and the directions are not symmetrical: a live document pointing at nothing is a 404 behind the staff Huddle button, silent until somebody taps it; an orphaned file costs storage and nothing else; reporting success for somebody ELSE's document sends an admin away believing their file is the one staff will open. Part of test:hygiene. Teeth-verified by six mutations
...
├── splash-watchdog.test.mjs ← the RECOVERY CONDITION in splash-watchdog.js. It fires when the module graph never runs — so it is the one code path that reports a fault nothing else can, and it had no test. Its condition used to be one line ("is the splash still up?"); it is not, because the splash now comes down BEFORE the access decision, so the watchdog was standing down at the exact moment the blank window opened. Same extraction technique as sw-internals: the file is a classic IIFE, so the function is sliced out by name and evaluated against its real source. Part of test:hygiene
...
├── auth-endpoints.test.mjs ← **who the reset-request notification reaches** — the one push in the app that must not be a broadcast, driven through the REAL handler against a fake Firestore, a fake Auth and a recording transport. It was pinned nowhere: the deploy surface says the endpoint EXISTS and the rules say who may READ the queue, while where the push GOES lived only in prose. Found in the wiring audit (v21.83): this repo tests its rules thoroughly and its wiring hardly at all. The fakes go into `require.cache` BEFORE the module loads, because it destructures `sendTargetedPush` at load time — patch it after and the suite passes while testing the real function. A static case pins the import surface too, since a fall-back-to-everyone could arrive down an error path no behavioural test walks. Runs in test:functions
...
├── overtime-data.test.mjs ← the CLOCK, and what a call says when it does not come back. `clockOffset` is pure and already pinned; what nothing could see is whether the offset it computes is ever FED — an offset dropped leaves `correctedNow()` reading the device clock, and a phone ten minutes fast hides Submit from somebody who is in time, with no error and nothing to distinguish it from a form that is simply closed. `fetch-timeout.js` is the REAL module and `globalThis.fetch` is the fake, so the three-way timeout/cancelled/network split under test is the shipped one. (--experimental-test-module-mocks). Teeth-verified by thirteen mutations — one of the first pass survived and turned out to be a mutation that changed no behaviour rather than a gap, which is worth knowing before trusting a count
...
├── playwright.webkit.mjs   ← the smoke suite under SAFARI'S ENGINE — desktop + Mobile Safari over the same specs (`npm run test:webkit`; add `-- --project=webkit` or `mobile-safari` for one). Runs in branch CI (`e2e.yml`) as ONE JOB PER ENGINE since v21.80 — sequentially it had become the thing every merge waited on at ~19 min, against siblings finishing in one to seven. Deliberately NOT the deploy gate. It is the ENGINE, not an iPhone. Why it exists and what it cannot see: the module header
...
npm run test:functions # Cloud Functions tests (roster-parse-helpers.test.mjs + functions-surface.test.mjs, which requires functions/index.js and pins the deploy surface, + overtime-endpoints.test.mjs + auth-endpoints.test.mjs) — not part of npm test (needs functions/node_modules)
