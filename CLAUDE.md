...
| Current app version | `11.57` (check `roster-data.js` — `APP_VERSION` is the authoritative source) |
...
| Three-surface model (v11.55) | Depth comes from three layered surfaces, defined in `shared.css :root`: **canvas** (`--surface-canvas` = navy `--primary-blue`, the page `body` on every page) → **card** (`--surface` = white panels that lift off the canvas via subtle shadows) → **sunken** (`--surface-sunken`, a faint just-cool off-white for recessed elements *inside* cards). Form fields rest on `--field-bg` (= sunken) and **brighten to `--surface` (white) on `:focus-visible`** — a "fills in when active" cue (applied to fields on index/admin/paycalc/operations). Disabled fields use `--field-bg-disabled` (a flatter, slightly darker NEUTRAL grey), kept deliberately distinct from the cool enabled tint so locked/disabled controls read as inert; admin's `#fieldMember:disabled` (locked-for-non-admins display box) intentionally uses `--field-bg`, not the disabled grey, so it reads as a normal field. **Always set field fills with `background-color` (longhand)** — `<select>`s layer their dropdown arrow via `background-image`, which the `background` shorthand would wipe. The navy canvas is intentional (the "navy background provides contrast" note on `--shadow-*`) — do not switch the body to a light canvas. |
...
| Calendar cell type hierarchy (v11.57) | **Shift dominates, date is context.** `.shift-time` (16px/700/--text-dark) is the primary cell element — what the staff member is working. `.day-number` is deliberately quietened: `font-weight: 500`, `font-size: var(--type-body)` (14px mobile baseline), `color: var(--text-mid)`, `font-variant-numeric: tabular-nums`. Desktop breakpoints reduce `.day-number` proportionally via the `--type-*` scale tokens. **Never re-bold the date number** — the old 20px/700 hierarchy was inverted. `.calendar-day:active .shift-badge { transform: scale(var(--press-scale)) }` uses the shared token so reduced-motion suppresses it automatically. |
