...
(async function initHuddleBanner() {
    try {
        const today  = formatISO(new Date());
        const huddle = await getTodaysHuddle(today);
        const banner = document.getElementById('huddleBanner');
        const btn    = document.getElementById('huddleOpenBtn');
        if (huddle && banner && btn) {
            banner.style.display = '';
            btn.addEventListener('click', () => window.open(huddle.storageUrl, '_blank', 'noopener'));
        }
    } catch (err) {
        // Silently degrade — the huddle banner is an enhancement, not a core feature
        console.warn('[Huddle] Could not fetch today\'s huddle:', err);
    }
})();
