    ...
    try {
        ...
        if (huddle) {
            btn.addEventListener('click', () => {
                // docx files are opened via Microsoft Office Online viewer rather than
                // the native PDF viewer — Office Online renders Word documents correctly
                // on any device without requiring conversion.
                const url = huddle.fileType === 'docx'
                    ? `https://view.officeapps.live.com/op/view.aspx?src=${encodeURIComponent(huddle.storageUrl)}`
                    : huddle.storageUrl;
