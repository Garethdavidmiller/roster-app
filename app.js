...
function renderTeamView(grade) {
    ...
    const currentBadge  = isCurrentWeek ? '<span class="tv-current-badge">This week</span>' : '';
    ...
    // Pass the week start time so the callback can discard stale results
    // if the user navigated to a different week before this fetch completed.
...
async function fetchTeamWeekOverrides(weekStart, weekEnd, fetchToken) {
    try {
        ...
        if (updated) renderTeamView(currentTeamGrade);
    } catch (err) {
...
(function initTeamLightboxes() {
    function wireOverlay(id, closeIds, onClose) {
        const lb = document.getElementById(id);
        if (!lb) return;
        function close() { lb.classList.remove('visible'); if (onClose) onClose(); }
    ...
    wireOverlay('teamComingSoonLightbox', ['teamCsClose', 'teamCsCloseBtn']);

    wireOverlay('teamInfoLightbox', ['teamInfoClose', 'teamInfoCloseBtn']);

    const printBtn = document.getElementById('teamInfoPrint');
    if (printBtn) printBtn.addEventListener('click', () => {
        document.getElementById('teamInfoLightbox')?.classList.remove('visible');
        window.print();
    });
})();
...
try {
        ...
        if (localStorage.getItem('myb_team_view') === '1') {
            const currentMember = getCurrentMember();
            if (CONFIG.ADMIN_NAMES.includes(currentMember.name)) {
                teamViewMode = true;
                applyTeamViewChrome();
                renderTeamView(currentTeamGrade);
            } else {
                localStorage.removeItem('myb_team_view');
                renderCalendar();
            }
        } else {
            renderCalendar();
        ...
        (function() {
            ...
            titleIcon.addEventListener('click', (e) => {
                e.stopPropagation();
                if (teamViewMode) {
                    const teamLb = document.getElementById('teamInfoLightbox');
                    if (teamLb) { teamLb.classList.add('visible'); return; }
                }
                openLightbox();
            });
