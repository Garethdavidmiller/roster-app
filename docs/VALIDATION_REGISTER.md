...
| **VAL-GUIDE-002** | **The FIP Kosovo caveat** — RSTL cannot verify whether Srbija Voz FIP facilities are still available at Elez Han — carried unresolved rather than decided either way. | RST or the operator, at the **Dec 2026** review (`fip-kosovo`) | A member is turned away abroad holding a document we said would work | `GUIDE_SOURCES.md` (`fip-*` rows) — **not restated here** |
...
written — and because three of these were closed by *disproving* the assumption.

| ID | Claim | Outcome |
|---|---|---|
| **VAL-GUIDE-002** (Irish Sea half) | The FIP Irish Sea coupon question | **Resolved at source, v20.22** — nil coupons, FIP Card 50% only, on the active-staff carrier table (`fip-stena-irish-sea`). The Kosovo half stays open above |
| **VAL-AUTH-006** | The Calendar's start wall is one `accounts:lookup` round trip — measured on an emulator with injected latency, with the FIELD contribution inferred rather than observed | **Confirmed on live staff devices**, 12 Sep 2026, on the signature the plan nominated in advance: `Recognised` runs 48% / 68% / 91% over a second across the not-reported / 4G-like / 3G-like groups while `Getting ready` is **0% over half a second in every one of them**. A network wall moves with the network and a parse wall does not. First read on 5 Sep at ~476 opens and closed here on 1,285, because a finding this size should not rest on a partial month — and the flat Getting-ready row is also what disposes of the platform-mix confound the 30 Aug read raised against the same two columns. `LATENCY.md` → *THE FULL-MONTH READ*. **And the question it left open is now answered** (19 Sep 2026): whether v22.97's fast path removed the wait could not be read off the September aggregate, because "rarely fires" and "fires and buys nothing" produce the same number. `readyProvisional` separated them — **1 open of ~800 eligible**, so the path rarely fires, the dilution explains the flat reading, and this confirmation stands unqualified. `LATENCY.md` → *THE CLOSING READ* |
