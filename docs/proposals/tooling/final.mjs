...
// Eight Forty's fits for B2's comparison, READ FROM ITS CELLS (25 Sep 2026). They came from
// eight-forty-table.json's `fit`, which table-book.mjs wrote from its own SEARCH objective — Saturday's
...
// (8.112 against 8.117 before rounding), so the typed claim was false the day it was written.
const satStanding = () => {
  const others = folderStats().filter(d => d.code !== codeFor(win) && d.sat !== null); if (!others.length) return `Saturday's fit is ${P.fits.sat}`;
  const best = Math.min(...others.map(d => d.sat)); const level = others.filter(d => d.sat === P.fits.sat).map(d => `<i>${d.name}</i>`);
  return P.fits.sat < best ? `Saturday's ${P.fits.sat} is the best in the folder` : P.fits.sat === best ? `Saturday's ${P.fits.sat} is level with ${level.join(' and ')}'s for the best in the folder` : `Saturday's ${P.fits.sat} is behind the folder's best, ${best}`; };
const meta = {
  otherTableFit, efFit: efFits,
