...
export async function renderPdf(D, out) {
  ...
  const F = {
    keep: { question: `Should the December link keep today's shift times, accepting ${QT ? 'a 15:45 closer and two 06:20 openers that run on' : 'one more cover cycle and nothing new to learn'}, or move to a table built from the December rules and the timetable?`,
    ...
    rules: { question: `Should the December link be built from the December rules and the measured timetable, accepting shift times nobody works today, or keep today's times?`,
    ...
    fifteen: { question: `Should the link be built on the supplied fifteen-turn table, accepting ${P.daily.sat} on a Saturday and ${P.daily.sun} on a Sunday against the December shape, for a table with the fewest distinct times here?`,
    ...
    line: { question: `Should the December link be the hand-built Weekday Lates line, in this revision, accepting ${P.daily.sat} on a Saturday against the agreed 14 and ${decMet} of the four December headcount rules met, for a week shape its authors chose?`,
  ...
  const cannot = [
    ...
    `Whether the December demand curve holds on Wembley event days — the Saturday weighting approximates them, and the curve is a measured timetable, not a footfall count.`,
    ...(sunWorse ? [`Why Sunday's fit (${P.fits.sun}) is worse than today's (${T.fits.sun}) and whether anyone minds: the December headcount of ten is met by adding people, not by reshaping the day.`] : []),
    ...(meta.frame?.cannot ?? []),
  ];
...
<section class="page frame">
  ...
  <div class="tiers">
    ...
    <div class="tier"><div class="tier-k">Soft — the room weighs these against each other</div>
      <ul><li>December headcounts met: <b>${decMet} of 4</b> (today 0 of 4)</li>
  ...
  <h2>How much each figure can bear</h2>
  <table class="t evid"><thead><tr><th>Figure on these pages</th><th>Where it comes from</th><th>Weight</th></tr></thead><tbody>
    ...
    <tr><td>Four at the open, three to close, 14 and 10 at the weekend</td><td>Owner-relayed December headcounts with no document behind them (class C)</td><td><b>Provisional</b></td></tr>
    ...
    <tr><td>Demand fit ${weekdayFitOf} · ${P.fits.sat} · ${P.fits.sun}</td><td>Measured timetable, one share-based measure on one December curve</td><td><b>Indicative</b></td></tr>
...
.tiers { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; } .tier { background: var(--surface-sunken); border-radius: var(--radius-sm); padding: 8px 10px; font-size: 9.5px; } .tier-k { font-weight: 800; color: var(--primary-blue); font-size: 9px; text-transform: uppercase; letter-spacing: .3px; margin-bottom: 4px; } .tier ul { margin: 0; padding-left: 14px; } .tier li { margin: 2px 0; }
...
<section class="page cover">
  ...
  ${meta.intro2 ?? `<p>It was not drawn by hand and it was not produced by the workspace's generator, which slides a person's start time through the week. Instead the December duty table was written in today's shift times, and a search then arranged those duties across 24 lines — over a million candidate arrangements — with the workspace's own hard-limit, rest, contract, fatigue and line-order modules scoring every one, plus a measure of how closely each resembled today's roster. Page 8 says how, and how to reproduce it.</p>`}`}
...
<section class="page">
  <div class="mast"><div><div class="eyebrow">${meta.eyebrow3 ?? (BB ? "The December table" : B2 ? "The December table, capped at 8h40, the ticket office rostered" : EF ? "The December table, capped at 8h40" : QT ? "Today's link, widened — the closer at 15:45" : "Today's link, widened")}</div><h1>${meta.h3 ?? (RULES ? "How a week compares with today" : QT ? "Same feel, later closer" : "Same feel, more people")}</h1><div class="sub">${meta.sub3 ?? (RULES ? "Nothing here was asked to resemble today's roster; this is how close the fatigue factors took it on their own, and where the December table differs" : QT ? "What the search was told to keep, what the December headcounts made it change, and the two openers that run on to pay for the later start" : "What the search was told to keep, and what the December headcounts made it change")}</div></div></div>
  <div class="stack">
  ...
  ${RULES ? `<h2>${BB ? "The duty table — the December default, beside today's" : "The duty table — the December rules under 8h40, beside today's"}</h2>
...
<section class="page">
  ...
  <p class="muted" style="margin-top:10px">Spare cover is not in these figures — a cover week carries no times, so the rows are a floor. <b>Fit</b> is the squared gap between each hour's share of the day's traffic and its share of the cover, inside the day's window — lower is better, and today's link scores what it scores. It is measured on duty minutes, so an hour a duty only touches counts in proportion; the cells above count heads. It is the measure the searched tables were chosen on, and the fit of the average weekday is the <i>Wk fit</i> on page 8. ${meta.sundayNote}</p>
