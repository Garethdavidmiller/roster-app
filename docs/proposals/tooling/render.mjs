...
<section class="page cover">
  ...
  <div class="sub">${meta.sub1 ?? (BB ? 'A 24-line link built from the December 2026 staffing rules and judged on the ORR fatigue factors — nothing carried over from today’s roster except the rules' : B2 ? 'The December 2026 timetable rules with no duty over 8h40 and the ticket office written in — two 14:00–22:30 turns a day Monday to Saturday, two 13:30–22:00 on a Sunday — judged on the ORR fatigue factors' : EF ? 'A 24-line link built from the December 2026 staffing rules with no duty over 8h40, judged on the ORR fatigue factors — nothing carried over from today’s roster except the rules' : PT ? 'Today’s roster, widened to 24 people, built to the owner’s pinned turns of 25 September 2026, the rest of each day fitted to the December 2026 timetable' : Q2 ? 'The existing 20-line link, widened to 24 people, with the closing turn at 15:45 and Saturday and Sunday rebuilt so that nothing runs over 8h40 — the same shape of week, one more cover cycle' : QT ? 'The existing 20-line link, widened to 24 people, with the closing turn starting at 15:45 — the same shape of week, one more cover cycle, and one turn a little longer to keep the contract' : 'The existing 20-line link, widened to 24 people — the same turns, the same shape of week, one more cover cycle')}</div>
  ...
  <div class="tiles">
    ${BB ? `<div class="tile"><b>${P.feel.distinctTimes} turns</b><span class="l">in the December 2026 duty table</span><span class="s">searched against the timetable; lates shorter than earlies; on the quarter hour except the open and close</span></div>`
         ...
         : PT ? `<div class="tile"><b>${sharedTimes} of ${P.feel.distinctTimes}</b><span class="l">shift times are today's</span><span class="s">the ${newTimes.length} that are not are the brief's own turns and the fitted middles — page 4 lists them</span></div>`
         : Q2 ? `<div class="tile"><b>${sharedTimes} of ${P.feel.distinctTimes}</b><span class="l">shift times are today's</span><span class="s">the ${newTimes.length} that are not: ${newTimes.join(', ')} — the later closer and the weekend's rebuilt turns</span></div>`
...
<section class="page">
  ...
  <p class="muted">${meta.dutyNote ?? `Why the extra duties land where they do: a weekday needs ${wkMinutes.toLocaleString('en-GB')} minutes and a Saturday ${satMinutes.toLocaleString('en-GB')} for 5 weekdays + 1 Saturday to total exactly 20 × 35h = 42,000 — an equality${PT ? ', and the split itself was searched (page 8)' : ", so the table is one of the few combinations of today's turn lengths that reaches it to the minute"}. ${Q2 ? "The weekday is <i>Quarter To</i>'s. The Sat and Sun columns are searched tables, not today's: every duty 7h–8h40, on a clock time somebody works today or the quarter hour, the day's headcounts and minutes held (page 8)." : QT ? "Today's three weekday openers — 06:20–13:35, 06:20–13:45 and 06:20–14:20 — fold into Saturday's two, 06:20–14:00 and 06:20–14:50; Sunday gains an 11:00–19:30 middle turn from the weekday vocabulary." : PT ? "The brief's pins are listed on page 6; every other duty was placed for the closest fit to the timetable under the day's headcounts, from today's clock times and the quarter hour (page 8)." : "The 06:20–13:35 turn is folded into 06:20–13:45 (ten minutes, one fewer time to remember); Sunday gains an 11:00–19:30 middle turn from the weekday vocabulary."}`}</p></div>
  </div>`}
  <div class="foot"><span>Page 4 of 9 — ${RULES ? "How a week compares with today" : PT ? "The brief, and what fitting did" : QT ? "Same feel, later closer" : "Same feel, more people"}</span><span class="foot-id"><b>${esc(meta.identity.name)}</b> · ${esc(meta.identity.code)} · ${esc(meta.identity.fingerprint)} · Marylebone Roster — Links designer</span></div>
</section>
