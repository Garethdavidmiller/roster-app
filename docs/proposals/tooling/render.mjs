...
export async function renderPdf(D, out) {
  ...
  const S = meta.stretch;   // { from, to, people, each, weekly, closer:{from,to}, closerShift } or null
...
<section class="page cover">
  ...
  <div class="tiles">
    ${BB ? `<div class="tile"><b>${P.feel.distinctTimes} turns</b><span class="l">in the December duty table</span><span class="s">searched against the timetable; lates shorter than earlies; on the quarter hour except the open and close</span></div>`
         : QT ? `<div class="tile"><b>${sharedTimes} of ${P.feel.distinctTimes}</b><span class="l">shift times are today's</span><span class="s">the ${newTimes.length} that are not: ${newTimes.join(' and ')} — the later closer, and the turn stretched to pay for it</span></div>`
  ...
  <p>A later start is a shorter turn. Thirty minutes off three duties a day is ${S?.closerShift ? S.closerShift * 3 * 5 : 450} minutes a week, and the contract is an equality, so those minutes have to go back somewhere — and with today's turns alone no table pays it. One turn is therefore stretched at its end: ${S ? `${S.from} becomes ${S.to} for the ${S.people} people on it, ${S.each} minutes each` : 'see page 3'}. That is the only other time on the sheet that is not one people work today. The search then arranged the duties across 24 lines exactly as <i>Same Turns</i> was built, with the workspace's own modules scoring every candidate. Page 7 says how, and how to reproduce it.</p>` : `  <p>A proposal for the CEA link once the December 2026 timetable arrives, for a team of 24 rather than 20. The brief was to keep the <b>feel</b> of the link people work today — a week is one turn, rest days come in pairs, four whole cover weeks spread round the rotation, and the same 06:20 / 08:00 / 14:00 / 15:15 clock times — while meeting the staffing shape agreed for December (four to open every day, three through to the close, four on a Saturday, five still on at 22:00, fourteen on a Saturday, ten on a Sunday) and every rule the link is assessed against: Chiltern's 13-day limit, twelve hours between duties, the exact 35-hour contracted week, and the ORR's good-practice fatigue factors.</p>
...
<section class="page">
  ...
  ${QT && S ? `<p class="muted">The 15:45 closer is ${S.closerShift} minutes shorter than the 15:15 turn it replaces — three a day, ${S.closerShift * 15} minutes a week — and with today's turns alone no table reaches 42,000 exactly. ${S.from} is run on to ${S.to.split('-')[1]} (${S.people} people, ${S.each} minutes each, ${S.weekly} a week) to put them back; the three other single-turn ways of doing it are on page 7.</p>` : ''}
...
<section class="page">
  ...
  ${BB ? `<p><b>1 · The table.</b> The workspace's own December default (<span class="tt">links-default-targets.js</span>): the owner's rules — four open, three close (four on Saturday), five on at 22:00, fourteen on a Saturday, ten on a Sunday, four cover weeks — with the times searched against the December demand curve under a strict late-shorter-than-early ordering, fewer distinct times, and the quarter-hour rule. It pays 20 × 35h exactly; the generator refuses any table that does not.</p>` : QT ? `<p><b>1 · The table.</b> <i>Same Turns</i>' table B, with the closing turn at 15:45. Every December duty table was re-enumerated from today's shift times with that one substitution — every count of each turn on each day class, opening with four, closing with three (four on Saturday), five on at 22:00 — and none totals exactly 42,000 Mon–Sat minutes: the later start takes 450 minutes a week out of a contract that is an equality. Letting one other weekday turn keep its start and finish 15, 30 or 45 minutes later, ${meta.tables} tables reach it, and four do so with every count exactly as table B has it. Two were carried forward and searched — <b>Q</b>, the 08:00 turn run to 17:00 (three people, thirty minutes each), and <b>R</b>, the 14:00 turn run to 23:15 (two people, forty-five each); the other two lengthen an opener and follow the December curve no better than table B. Table <b>${meta.winnerVariant}</b> won, on the rules first and the fit second. Saturday and Sunday are <i>Same Turns</i>' own (<span class="tt">table-late.mjs</span>).</p>` : `<p><b>1 · The table.</b> Every possible December duty table was enumerated from today's shift times — every count of each turn, on each day class — and kept only if it opened with four, closed with three (four on Saturday), kept five on at 22:00, and totalled exactly 42,000 Mon–Sat minutes. ${meta.tables} tables survive. Two weekday shapes were carried forward — table A with three 11:00 middles, table B with a third 08:00 and today's 13:30 turn — and every finished design was scored against the December demand curve; B, used here, follows a weekday's curve about as evenly as today's roster does (57.7 today, 58.5 here, lower is more even), where A follows it less well (69.5) — the three 11:00 middles put nine people on the quietest part of the afternoon.</p>`}
