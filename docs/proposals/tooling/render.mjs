...
`;
  ...
  const OTHERS = folderStats().filter(d => d.name !== meta.identity.name).map(d => ({ ...d,
  ...
  const todayWk = meta.alternatives?.find(a => /Today/.test(String(a.name)))?.fit ?? T.fits.tue;
  ...
  const MEASURES = [
    { q: 'Does it follow the trains?', name: 'Demand fit — weekday', lower: true, lo: 0, hi: Math.max(90, Math.ceil(((EXT.wk?.worst ?? 0) + 5) / 10) * 10, Math.ceil((wkFitP + 5) / 10) * 10), fmt: v => v, val: wkFitP, today: todayWk, e: EXT.wk, page: 6,
      what: 'How closely the people on duty follow the trains through an average weekday. For each hour, its share of the day’s staff-minutes is compared with its share of the timetable’s traffic; the gaps are squared and added up. 0 would be a perfect match; a score near 25 means each hour is about one percentage point off its share, near 50 about one and a half.',
    ...
    { q: 'What is it like to work?', name: 'Full weekends off', lower: false, lo: 0, hi: 50, fmt: v => v + '%', val: P.checks.weekendsOffPct, today: T.checks.weekendsOffPct, e: EXT.weekendsPct, page: 5,
      what: 'The share of weeks with both Saturday and Sunday off. Today’s link gives 4 weekends in 20; a 24-line link with six is the same one weekend in four.',
