...
export async function renderPdf(D, out) {
  ...
  const EF = meta.kind === 'EF'; const RULES = BB || EF;
  const ef = EF ? (() => {
...
<section class="page">
  <div class="mast"><div><div class="eyebrow">${BB ? "The December table" : EF ? "The December table, capped at 8h40" : QT ? "Today's link, widened — the closer at 15:45" : "Today's link, widened"}</div><h1>${RULES ? "How a week compares with today" : QT ? "Same feel, later closer" : "Same feel, more people"}</h1><div class="sub">${RULES ? "Nothing here was asked to resemble today's roster; this is how close the fatigue factors took it on their own, and where the December table differs" : QT ? "What the search was told to keep, what the December headcounts made it change, and the two openers that run on to pay for the later start" : "What the search was told to keep, and what the December headcounts made it change"}</div></div></div>
  <div class="stack">
  ...
  <div class="foot"><span>Page 3 of 8 — ${RULES ? "How a week compares with today" : QT ? "Same feel, later closer" : "Same feel, more people"}</span><span class="foot-id"><b>${esc(meta.identity.name)}</b> · ${esc(meta.identity.code)} · ${esc(meta.identity.fingerprint)} · Marylebone Roster — Links designer</span></div>
</section>
