...
| **EXC-007** | **Document bearer URLs already in circulation stay live.** Since v23.17 the Huddle, Circular and Newsletter are behind the PIN or a password in the app, and their Firestore reads were closed at v23.18; but each file is fetched by a permanent tokenised Storage URL that bypasses rules, so anyone who obtained one before (browser history, a forwarded link, Microsoft's Office viewer cache) can still open that file. New readers cannot discover a URL; old holders keep theirs. | `storage.rules` header · AUTH_PLAN.md §5 (E6) | **The client calls `getDocumentUrl` and the existing objects are rotated.** The Office viewer STAYS — the owner refused both routes that would have removed it (AUTH_PLAN §5, DECIDED 19 Sep 2026), so the closure condition is no longer "replace the delivery for `.docx`"; it is that the URL handed to that viewer becomes short-lived. The SERVER half is built and deployed (v24.16–18); what keeps this row open is that the three opening surfaces still read the stored permanent URL, and that rotation — the only step that reaches an object already orphaned by a partial prune — has not been taken |
