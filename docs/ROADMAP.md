...
| Is a Daily Huddle's TEXT confidential? | **DECIDED 7 Sep 2026 — yes: the Huddle, the Circular and the Newsletter are not visible without the PIN or a password.** Built in two releases: the client stops reading without access first (v23.17, `calendar-doc-access.js` — the local cache is why a rule alone could not do it), the rules closed the three collections' reads at v23.18. What this does NOT do is revoke bearer URLs already in circulation (ARCHITECTURE EXC-007; AUTH_PLAN §5 E6). Was: recorded only in a `storage.rules` comment until this row (v23.05, measured 6 Sep 2026). The file needs its bearer URL, but since DOCX ingest the converted words are also written to the `huddles` document as `htmlContent`, and `firestore.rules` grants that collection a bare `allow read;` — an unauthenticated REST read returns them (the same request against `overrides` is denied). Nothing is broken and nothing changed; the caution in the rules file was simply narrower than the exposure. The decision is whether a day plan may stay world-readable or should sit behind the Calendar's `name`/`calendarViewer` claim like the overrides do — and it has consequences: the Huddle auto-opens on fresh visits with no session, which is why the collection was left open when v20.12 closed the roster. Owner's call; until it is made, treat the text as public |
