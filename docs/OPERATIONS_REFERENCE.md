...
- `"NA"` / `"N/A"` = not available = **Absent** — the model returns the code and the server records `SICK` Monday to Saturday and `RD` on a Sunday, where it is a clerical error and no absence can be held (v24.22). `"NS"` = not available on a **Sunday**: `RD` on Sunday, a review question on any other day. Until v24.22 the first was read as a Sunday-only code, on a misheard owner remark
