...
| 14 | **A personal action is offered only to the person it belongs to.** The day panel's "Leave dates" and "Pay estimate" buttons open the reader's OWN records, so they appear only where the calendar on screen is the signed-in member's own — never on a colleague's day, and never in viewer mode, which is a separate refusal because a stale session can outlive the identity that earned it. | `calendar-access-core.js` (`personalActionsAllowed`) · `calendar-al-lightbox.js` |
