...
| ~~28 Sep 2026~~ ~~mid-Oct 2026~~ **nothing to diary** | ~~end Oct 2026~~ | **ALL FOUR READINGS ARE TAKEN — the fourth on 19 Sep 2026, and `LATENCY.md` IS CLOSED.** The question was whether v22.97's fast path removed the wait, and the September aggregate could not tell "it rarely fires" from "it fires and buys nothing" — same samples, opposite conclusions. `readyProvisional` separated them: **1 open of ~800 eligible**. That is row one of the table the plan pre-registered, so the identity finding stands, `VAL-AUTH-006` is not reopened, and what is left to `Recognised` is recorded as the price of the security model. **The read came a month EARLY and is still decisive** — it was diaried for mid-Oct on the reasoning that a new metric reports only from updated devices, which is true and did not need the extra month at this margin. **Do not open a Phase 5**: the file's own closing rule forbids it by name, including the refusal-reason counter that would explain the 1, because widening the path's eligibility is an ACCESS decision and not a latency one | `LATENCY.md` → *THE CLOSING READ* · `VAL-AUTH-006` |
| ~~21 Nov 2026~~ **nothing to diary** | ~~Dec 2026~~ | **The Overtime retention purge ARMS ITSELF on 1 Dec 2026** (v24.10, owner decision — this row used to say "set `purgeArmed: true` in `functions/index.js`", which is no longer a thing anyone does). The rule is `purgeArmedAt` in `functions/overtime-core.js`. **The dates are kept because they are still the ones that matter, and because this row's own history is the argument for the change**: they were RELATIVE until Aug 2026 ("10 weeks after the first Overtime window"), which got misread in both directions; then computed — the scheduler's first run on 11 Aug 2026 made weeks ending 22 Aug – 12 Sep, retention is 91 days past the week-ending Saturday, so **the earliest window expires 21 Nov 2026**. A hand-flipped boolean could not be right on either side of that: before it there is nothing to read, and ON it the first window would be deleted the morning it appeared. The date sits ten days after, so runs from 21 Nov to 1 Dec name real windows and delete nothing. **Reading one is now OPTIONAL confirmation (`VAL-OT-001`), not a gate** — look for `[purgeExpiredOvertimeWindows]` in the Functions log if you want it | `OVERTIME_AVAILABILITY.md` → Retention · `VAL-OT-001` |
| **Mar 2027** | **Her return — date not yet known** | **S. Faure's return to the rotating link, and where B. Toth goes.** She came off the rotating dispatcher link on 29 Jun 2026 onto `fixedRoster[2]` (Mon–Fri 09:00–16:00) via a `rosterChanges` entry, and **there is no third entry taking her back** — so she stays on that row indefinitely, and her calendar quietly shows a 09:00–16:00 week she is not working once the absences stop. Two coupled decisions, not one: B. Toth holds line 4 now, so her return needs a line for one of them. The absences are the warning bell — the booking tool caps a range at one year, so whatever was recorded from 29 Jun 2026 runs out by **29 Jun 2027 at the latest** and the roster silently reverts to looking like ordinary work | `roster-member-data.js`, re-exported by `roster-data.js` (both entries carry the reasoning) |
...
|---|---|---|
| **Oct 2026** | `fip-eurostar-fares` — the Eurostar Blue FIP Card fares | `GUIDE_SOURCES.md` |
| **Nov 2026** | **Railcard:** the ten `rc-*` rows (Marylebone off-peak and restrictions, forgotten Railcard, 16-17 Saver and seasons, Network easements, digital display, Jobcentre Plus and its validity, London discounts) | `GUIDE_SOURCES.md` |
| **Nov 2026** | **Rangers & Rovers:** the eight `rr-*` rows (Shakespeare Explorer and its break of journey, the Thames Rover operator question, Cotswolds, Severn & Solent, Two Together, the date box, advance sale) | `GUIDE_SOURCES.md` |
| **Nov 2026** | **FIP money and booking:** `fip-booking-table`, `fip-sncf-payment`, the five supplement rows (`fip-at`/`-es`/`-pl`/`-sk-supplements`, `fip-cz-supplement`), `fip-eurostar-rules`, and the ferry rows `fip-stena-north-sea`, `fip-stena-irish-sea`, `fip-attica` | `GUIDE_SOURCES.md` |
| **Dec 2026** | **FIP countries:** every per-country row (`fip-fr` … `fip-ni`, including `fip-ch-mountain`, `fip-hu-gysev`, `fip-lake-constance`) plus `fip-carrier-accept` and `fip-kosovo` — the Kosovo caveat is the one open question (`VAL-GUIDE-002`) | `GUIDE_SOURCES.md` |
| **Jan–Feb 2027** | The rest of the register — FIP rules rows, the remaining railcard and Rangers rows | `GUIDE_SOURCES.md` |
...
The guide rows above are copied from the register's `Next` column (26 Sep 2026); where they differ,
the register wins — re-derive them from it rather than editing a date here.
