...
| **28 Sep 2026** | **mid-Oct 2026** | **Read the latency numbers, then settle the plan.** One card visit answers all of them: (1) "What put the shifts on screen" — the cache/server split that decides Phase 2 (a small server-served share CLOSES it; substantial and slow starts it, narrowest form first), shipped v21.99 so a full month lands ~28 Sep; (2) ~~the field-confirmation check gating the identity decision~~ — **taken 5 Sep 2026: the signature was present, the decision was made and the fast path shipped in v22.97. What the September visit reads instead is whether it WORKED** — `Recognised` should stop gating `Shifts shown` for cache-served starts while `Getting ready` is unchanged; no movement means the diagnosis was wrong ; (3) **did the cached-roster fast path work?** — it shipped v22.97 on 6 Sep, so this is the first honest reading: `Shifts shown` should stop waiting on `Recognised` for cache-served starts while `Getting ready` is unchanged, and no movement means the diagnosis was wrong; (4) **is the service-worker revalidation storm costing anything?** — `swrCount` began recording 5 Sep and its App Speed readout landed 6 Sep, so the same visit answers whether heavy-worker boots are slower than the rest. **A reading taken before this date is a few days of samples, and 3 and 4 both shipped the same week as this row was written** — which is exactly how a fix gets declared successful on a measurement of the weekend. Both reading rules are written in `LATENCY.md`; the plan's own close-out says what happens on each answer. **The second one was NOT readable until v22.28** — the card could only split `domReady` by connection, i.e. the milestone the evidence exonerates — so it now has its own block, and because the dimensions have been recorded since v21.30 it reads against August as well as September. **The deadline is soft but real**: samples roll off month by month, and a reading nobody takes is how a plan fades instead of closing | `LATENCY.md` · `ROADMAP.md` → the identity round trip |
