...
| ~~28 Sep 2026~~ **mid-Oct 2026** | **end Oct 2026** | **THREE OF THE FOUR READINGS ARE TAKEN — 12 Sep 2026, on 1,285 Calendar opens.** Phase 2 closed (98.4% cache-served, 2.8× the sample it was closed on); the field-confirmation check CLOSED `VAL-AUTH-006` (`Recognised` 48% / 68% / 91% over a second across the connection groups, `Getting ready` **0% in every one**); the service-worker storm exonerated (a full sweep on 78% of opens, and those boots are no slower than any other). **The fourth is the one still open, and the reason is worth keeping: it was not readable, not merely early.** Did the v22.97 fast path work? `Saved copy` went 78% over a second → 77%, and that single figure fits BOTH "the path rarely fires" (the diagnosis stands) and "it fires and buys nothing" (the diagnosis is wrong) — opposite conclusions, and **more months of the same aggregate could never separate them**, which is what this row's original wording assumed. `readyProvisional` was added at v23.70 to measure the share directly, so the new clock runs from that release, not from v22.97. **Read it no earlier than mid-Oct**, against the decision table in `LATENCY.md` → *THE FULL-MONTH READ* → item 6. The deadline is soft but real: samples roll off month by month, and a reading nobody takes is how a plan fades instead of closing | `LATENCY.md` · `VALIDATION_REGISTER.md` |
