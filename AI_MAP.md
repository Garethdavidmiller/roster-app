...
- `generateLink({ slots, spareLines = 0, lines = 28, settled = true })` → `{ patterns, mode, waves, reason }` — the slot-based generator (the only way to create a new design); `generatePatterns(opts)` is the patterns-only wrapper keeping the old signature. **The parameter is `spareLines` and the default construction is SETTLED WEEKS** — this line said `spare` and "rotating-window" until v19.65, contradicting the bullet below it in its own entry (v19.58 made spare a count of whole lines; v19.59 changed the default)
...
- `rosterSeedLines()` → the lines sampled: the WHOLE main cycle then the WHOLE bilingual cycle, lengths read from `CONFIG.MAIN_ROSTER_WEEKS`/`BILINGUAL_ROSTER_WEEKS`.
...
- **The bug it guards.** v19.59: the seed took main's 20 weeks plus only the two bilingual weeks the two bilingual members happen to sit on, so bilingual 1 and 8 — the SPARE weeks — were never sampled and the seeded spare count came back **4** against a true **6**: two whole lines of standby cover missing from every design that started from it. Silent, because every per-shift headcount was correct; only the spare-LINE count was wrong, and nothing shows that beside its expected value.
- **The latent repeat, now closed.** The coordinator hardcoded `w <= 20` / `w <= 8` while CONFIG held the same numbers, so a roster changing length would have left the seed reading a prefix of it — the same shape, again, silently. A static assertion in the test bans a bare numeric loop bound.
...
- `MAX_CONSECUTIVE_WORKED_DAYS` (13 — from the **Hidden report** into the Clapham Junction crash, 1988; an industry limit, not legislation and not a house rule: see v19.90 below) / `assessHardLimits(patterns, lines)` → `{ checks, breaches, assessable }`
