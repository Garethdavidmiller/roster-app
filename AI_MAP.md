...
- `dismissLoginOverlay()` — **in-place sign-in (Phase 9, v14.81).** Removes the `#loginOverlay` element + `unlockBodyScroll()`; no-op when no overlay is mounted (so it is safe to call unconditionally on a coordinator's authorised path). When `CONFIG.INPLACE_LOGIN` is **on**, the init()-wrapped coordinators (operations/links/paycalc) pass `onSuccess: () => init()` instead of `reload()` — re-entering `init()` runs the authorised body (which never ran on the login pass) exactly once and calls `dismissLoginOverlay()`; they also skip `resolveSession(false)` on the login pass so the one-shot `sessionReady` isn't poisoned before the in-place pass resolves it. Flag **off** (default) = today's reload, unchanged. See ARCHITECTURE_PLAN.md → "Phase 9".
