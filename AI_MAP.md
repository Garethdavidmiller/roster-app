...
- `getPerfStats()` — admin-only read for the Operations "App speed" card (v14.90; two journeys v14.92): reads `analytics/perf_<YYYY-MM>` and returns `{ month, login, pages }` — each a plain-language `summarisePerf` summary (overall + per-page quick/ok/slow bands) for the `loginTotal` and `domReady` metrics respectively. The card render (`initPageSpeedCard` in `operations-app.js`) shows a "🔑 Signing in" verdict + a "📄 Opening pages" verdict + per-page speed bars; bucketing/verdict maths is the pure `perf-stats.js` module.
