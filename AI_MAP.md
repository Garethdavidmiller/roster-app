...
| Override priority, member-start, rest-shift helpers — tsToMillis, shouldReplaceOverride, foldOverrideIntoCache (Team-View merge), isBeforeMemberStart, isRestShift, isOverrideDisplaySuppressed, resolveEffectiveShift (shared display ladder), computePeriodDeleteIds; training value grammar + pay resolver | `override-utils.js` |
...
- `foldOverrideIntoCache(existing, incoming)` (v16.65) — the Team-View Firestore-merge decision, returning `{ store, displayChanged }`. ALWAYS store when the incoming record out-ranks the cached one (so the winner's `source`/`createdAt` is cached, not a stale same-value loser's) and gate the re-render on `displayChanged` only. Extracted from `calendar-team-view.js` so the v16.63 fix (a same-value manual must not leave an import's metadata behind for a later import to out-rank) is unit-tested. Tested by `override-utils.test.mjs`.
