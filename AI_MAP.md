...
- `buildRosterTargets(sources = rosterSeedLines())` → `{ slots: [{time, weekday, sat, sun}], spareLines }`. Weekday is the **busiest** Mon–Fri day for that time (some shifts run Tue/Thu/Fri only, and under-staffing is the worse error — hence the table's `busiest day` header); spare is counted in whole **lines**, never per day.
...
- **The bug it guards, and the rule behind it.** The rule is: **the seed samples exactly what the design represents.** It has now been applied twice, in opposite directions, and both failures were silent.
  ...
  - *v19.98 — too WIDE.* The design is now 22 lines of the main roster and excludes the bilingual roster, so seeding from it would target **ten shift times no line in the design can work** — all ten bilingual times are bilingual-only, zero overlap with main's 18. Main-only seeds **18 slots, 4 spare lines** (main 1/7/12/17).
