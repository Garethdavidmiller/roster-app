...
- **Account status card** `initAccountStatus()` (v18.63, PASSWORD_PLAN.md Track C) — joins `getAllStaffContacts` + `getAllPasswordStatus` (both admin-only reads, under `withClaimRetry`) and lists every active member with an Email ✓/— flag, a Password 🔑✓ (self-set) / 🔑surname (default) flag, and a **Reset** button. "Migrated" = `passwordSetAt` present AND `≥ resetAt`. Reset opens a `confirmDialog` then calls `resetMemberPassword(name, { revoke: true })` (admin break-glass → surname default + refresh-token revoke), then re-reads status and re-renders. Fails to `_cardLoadError` with a retry.
