...
| Override priority, member-start, rest-shift helpers — tsToMillis, shouldReplaceOverride, isBeforeMemberStart, isRestShift, isOverrideDisplaySuppressed, computePeriodDeleteIds; training value grammar + pay resolver | `override-utils.js` |
...
- `writeWithClaimRetry(writeFn)` — runs a Firestore WRITE thunk and, on a stale-claim `permission-denied` with a live user, force-refreshes the ID token (`getIdToken(true)`) once and retries once (write-side mirror of `operations-app.js` `adminReadWithRetry`; SECURITY_RELEASE_PLAN.md → B3 hardening, v15.18). The thunk must rebuild its batch each call (a `WriteBatch` can't be re-committed). Used by `recordRangeOverrides` in `admin-overrides.js` so a just-provisioned manager's on-behalf AL/absence/shift write self-heals instead of erroring.
...
- `isOverrideDisplaySuppressed(override, baseShift, sunday)` (v16.37) — **SINGLE SOURCE** for the calendar display-suppression rule (CLAUDE.md "Sundays are non-contracted", layer 5): true when an override must NOT replace the base shift (a `sick` on a rest-day/Sunday base; `annual_leave`/`other` on a Sunday). Used by `calendar-renderer.js`, `calendar-team-view.js`, and `getShiftTypesInMonth` (calendar-overrides.js) so the three can never drift — replaced 4× hand-written copies (this also fixed the team view's previously-missing Sunday-AL suppression). Takes `sunday` as a boolean (callers pass `isSunday(dateStr)`) to avoid a roster-data import cycle. Tested by `override-utils.test.mjs`.
...
- `linkDesigns` read: open to any authenticated session. Write: **`token.linksDesigner == true || token.admin == true`** (H2, shipped v16.29) — the `linksDesigner` claim is set by `setupRosterAuth` from `CONFIG.LINKS_DESIGNERS`; `links-app.js` wraps `linkDesigns` writes in `writeWithClaimRetry` so a stale token self-heals. Before v16.29 any authenticated identity could write any Links design.
