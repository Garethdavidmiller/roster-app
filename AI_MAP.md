...
| Links design workspace — the rotation design grid (`ROTATING_LINES`, 22 since v19.98), paint mode, hourly coverage heat map, design checks, auto-generator UI | `links-app.js` + `links.html` + `links.css` |
...
- `DEFAULT_WINDOW` (Mon–Sat 06:20–23:55 · Sun 07:15–23:25) · `normaliseWindow` · `windowForDay` · `isHourStaffed` · `heatSpan` · `formatWindow` · `windowsDiffer` · `isDefaultWindow` · `isValidWindowRow` · `windowMinutes` · `canonicaliseWindowTime`.
...
- **Deterministic**, no randomness anywhere: a designer can re-run it and get their design back, and two designers comparing notes are comparing the same thing. Greedy nearest-neighbour then a bounded 2-opt — not optimal over 28 nodes and it does not pretend to be.
...
- **Country finder** (v17.64): a search box (`#countrySearch`) live-filters the 25 country cards AND the A–Z jump chips by country name OR operator/train text — `applyCountryFilter(raw)` matches against each card's full text, toggles `.hidden`, and updates the count / no-match live regions.
...
### `functions/index.js`
Eight Cloud Functions (Firebase-dependent shell — pure logic lives in `roster-parse-helpers.js`):
...
- `onCircularCreated` / `onNewsletterCreated` — Firestore `onDocumentCreated` triggers on the `circulars` / `newsletters` collections; each fans out its own document-arrival push (📰 Circular / 🗞️ Newsletter) via `buildPushPayload` + `fanOutPush`
...
- `resetMemberPassword` (v18.63, PASSWORD_PLAN Track C) — admin-only break-glass: verifies the caller's admin claim, resets a member's Firebase Auth password to their surname default (`nameToPassword`), optionally revokes their refresh tokens (`revoke`, default true), and stamps `passwordStatus.resetAt` (Admin SDK, bypasses rules). CORS-restricted to `ADMIN_FUNCTION_ORIGINS`
