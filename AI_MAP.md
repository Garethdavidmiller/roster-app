...
- One-shot fetch (`getLatestCircular`/`getLatestNewsletter`) on open — no persistent subscription (unlike the Huddle, which needs live state for its button). Reached from **both** the ☰ nav-drawer links (now plain `./index.html#circular` / `#newsletter` links, like Daily Huddle) **and** Circular/Newsletter notification taps — both deep-link to the same viewer (v14.56).
...
- `getLatestCircular()` — queries `circulars` collection, returns latest doc's data (with `storageUrl`) or null; called from `calendar-doc-viewer.js` (v13.58; moved from nav-panel v14.56)
...
- `getLatestNewsletter()` — queries `newsletters` collection, returns latest doc's data (with `storageUrl`) or null; called from `calendar-doc-viewer.js` (v13.59; moved from nav-panel v14.56)
