...
- `scaleTargets(slots, pct)` → `{ slots, before, after }` (v19.78) — the December 2026 uplift: scale the generator's target table, keeping each shift's share. **Rounds the day-class TOTAL and distributes the remainder (largest-remainder), never each slot** — the targets are mostly 1s and 2s, so per-slot `Math.round` made **every uplift from 0% to 15% byte-identical** against the real seed (104 duties, unchanged) and a designer typing 10% would see a control that appeared broken. **A slot at 0 for a day class stays 0**: zero means that shift does not run that day, and promoting it would invent a duty and change the shape package 4 exists to preserve. Deterministic tie-break, so the same uplift always gives the same table. Project context + the capacity finding it feeds: `LINKS_DEC2026_PLAN.md` package 4.
