...
*Last updated: July 2026 — v15.60 · Updated every 0.10 version*
...
- `logClientError(data)` / `getClientErrors()` / `resolveClientError(id)` — `clientErrors` collection (v13.31); `logClientError` called from `error-reporter.js`, read/resolve called from `operations-app.js`. Ordering/retention policy delegated to `client-errors.js` (v13.48). `getClientErrors()` returns `{ errors, truncated }` — `truncated` is true when the (index-free, no-`orderBy`) unresolved query hit its 100-record cap, so the Error Log card shows a "showing the first 100" banner instead of silently hiding the rest (no-silent-caps; v15.69).
