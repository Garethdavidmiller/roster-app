...
- `EXTRA_SPARE_WEEKS` (v20.01) — the cover week the WIDENED link carries over what the roster provides. Its own constant because it is a number the roster cannot tell you: everything else in the table is an observation, this is a decision. Named WEEKS not LINES both because that is what the UI and docs call it and because `links-rotation-parity.test.mjs` flags any `const *LINES* = <number>` in a Links module.
- `buildRosterTargets(sources = rosterSeedLines(), { extraSpareLines = EXTRA_SPARE_WEEKS } = {})` → `{ slots: [{time, weekday, sat, sun}], spareLines, rosterSpareLines }`. **Both spare figures are returned** — `rosterSpareLines` is measured, `spareLines` is that plus the uplift — so the observation and the decision can never be confused, and `{ extraSpareLines: 0 }` still answers "what does this roster provide?", which is the question the v19.59 bug got wrong. Weekday is the **busiest** Mon–Fri day for that time (some shifts run Tue/Thu/Fri only, and under-staffing is the worse error — hence the table's `busiest day` header); spare is counted in whole **lines**, never per day.
...
- **The bug it guards, and the rule behind it.** The rule is: **the seed samples exactly what the design represents.** It has now been applied twice, in opposite directions, and both failures were silent.
  ...
  - *Neither later change is visible in the sampling, and that is the design working.* The rotation went 22 → 24 at v20.01 and gained a spare week; the SLOTS are an observation about the roster and do not care how long the design is, while the added spare week is a decision and lives in its own constant.
...
- `OBJECTIVES` · `GENTLE_THRESHOLD_MINUTES` · `DEFAULT_BLOCK_TARGET` · `lineStartProfile` · `weekStep` · `breakLength` · `boundaryRest` · `blockRuns` · `blockExcess` · `scoreOrder` · `cost` · `reorderLines` · `applyOrder`.
...
- Applied by `links-app.js` after Generate, behind **five** checkboxes in the generator card (`variety` joined the original four at v19.60 — both counts here said "four" until v19.65). Tested by `links-adjacency.test.mjs`.
